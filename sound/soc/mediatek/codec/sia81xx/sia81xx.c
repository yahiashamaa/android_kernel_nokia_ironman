...
//Bug589753 , liujun5.wt, 20201022, add mmitest and smartpa info
 ...
 ********************************************************************/
//bug584797 liujin.wt, modify, 20201019, change pa mode 
#define AW87318_PA_MODE 3
void Awin_Speaker_Amp_Set(int extamp_gpio, bool enable)
{
    int i;
    if(enable) {
        msleep(1);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if(is_sia81xx_pa) {
		...
		if(1 == ucontrol->value.integer.value[0])
			Awin_Speaker_Amp_Set(sia81xx->rst_pin, true);
		else
			Awin_Speaker_Amp_Set(sia81xx->rst_pin, false);
	}
...
static int sia81xx_audio_scene_set(
	...
	struct snd_ctl_elem_value *ucontrol)
{
    //Bug601180 liujun5.wt add 20201117, distinguish PA type aw87318 or sia81xx
    if(is_sia81xx_pa){
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	sia81xx_dev_t *sia81xx = snd_soc_component_get_drvdata(component);
...
//bug584798, liujin.wt, add, 20201112, add factory
	...
	const struct i2c_device_id *id)
{
	...
	if (1 == check_sia81xx_status(sia81xx)) {
		...
		is_sia81xx_pa = true;
                ...
                hardwareinfo_set_prop(HARDWARE_SMARTPA_IC_INFO, "SIA8101");    //Bug589753 , liujun5.wt, 20201022, add mmitest and smartpa info
		...
		is_sia81xx_pa = false;	
                ...
                hardwareinfo_set_prop(HARDWARE_SMARTPA_IC_INFO, "AW87318");    //Bug589753 , liujun5.wt, 20201022, add mmitest and smartpa info
