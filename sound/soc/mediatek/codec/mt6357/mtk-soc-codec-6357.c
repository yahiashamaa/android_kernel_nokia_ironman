...
static int Voice_Hac_Set(struct snd_kcontrol *kcontrol,
                        struct snd_ctl_elem_value *ucontrol)
{
    mutex_lock(&Ana_Power_Mutex);//Bug589757 liujun5.wt modify 20201118 for baseline update causes error,update Ana_Ctrl_Mutex to Ana_Power_Mutex
    pr_debug("%s() value = %ld\n ", __func__,
        ucontrol->value.integer.value[0]);
    if ((ucontrol->value.integer.value[0] == true) &&
        (mCodec_data->mAudio_Ana_DevicePower
    ...
    mutex_unlock(&Ana_Power_Mutex);//Bug589757 liujun5.wt modify 20201118 for baseline update causes error,update Ana_Ctrl_Mutex to Ana_Power_Mutex
    return 0;
}
//-IRM-17045 xiangyujie.wt add 20200914 for add voice hac feature
