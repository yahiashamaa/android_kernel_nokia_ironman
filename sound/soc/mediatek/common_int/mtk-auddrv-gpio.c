...
//+bug584860 xiangyujie.wt 20200914 add, audio bringup
...
//+bug584860 xiangyujie.wt 20200914 add, for awinic pa timing sequence less than 10us
...
{
	int i;
	//int ret=0;//Chk36598 liujun5.wt modify 20191017 for touch sound is different
	...
	if(enable) {
		...
		for(i=0; i<AW87318_PA_MODE; i++) {
			...
			//ret=gpio_get_value(extamp_gpio);//+Chk36598 liujun5.wt modify 20191017 for touch sound is different
			//pr_err("cjdon1=%s@%d", __func__, ret);//-Chk36598 liujun5.wt modify 20191017 for touch sound is different
			ndelay(1000);
			//gpio_direction_output(extamp_gpio, 1); // more than 10us
			gpio_set_value(extamp_gpio, 1);  // less than 10us
			//ret=gpio_get_value(extamp_gpio);//+Chk36598 liujun5.wt modify 20191017 for touch sound is different
			//pr_err("cjdon=%s@%d", __func__, ret);//-Chk36598 liujun5.wt modify 20191017 for touch sound is different
		}
		preempt_enable();
		usleep_range(13 * 1000, 15 * 1000);//Chk36598 liujun5.wt modify 20191017 for touch sound is different
	}
	else {
		gpio_direction_output(extamp_gpio, 0);
