...
//+ Bug584789, lishuwen@WT,20200923, add, google key bring up
...
{
	const volatile void __iomem *kp = kp_base;
	state[0] = readw(kp + KP_MEM1);
	state[1] = readw(kp + KP_MEM2);
	state[2] = readw(kp + KP_MEM3);
	state[3] = readw(kp + KP_MEM4);
	state[4] = readw(kp + KP_MEM5);
...
//+bug585487,wangyongfei_wt,20200917,add,google-key
	...
	tasklet_init(&keypad->tasklet, kpd_keymap_handler, (unsigned long)keypad);
