...
{
 s32 returnValue = 0;
 LOG_INFO("abov_sar_cali!\n");
 returnValue = write_register(this, ABOV_RECALI_REG, 0x01);
 LOG_INFO("abov_sar_cali:returnValue=%d\n",returnValue);
 return returnValue;
}
...
static int capsensor_set_enable(struct sensors_classdev *sensors_cdev, unsigned int enable)
{
 ...
 if (enable == 1) {
  ...
  //initialize(this);
        ...
        //manual_offset_calibration(this);
//+Bug591052 tangsha.wt, MOD,20201012, add sar sensor chip id to factory interface
        manual_offset_calibration(this);
        msleep(500);
//-Bug591052 tangsha.wt, MOD,20201012, add sar sensor chip id to factory interface
  //+Bug 436909 huangcunzhu_wt,20190416,MODIFY sar work when sleeping
...
//+Bug591052 tangsha.wt, MOD,20201012, add sar sensor chip id to factory interface
static ssize_t sar_id_show(struct class *class,
  struct class_attribute *attr,
  char *buf)
{
  u8 chip_id = 0;
 ...
 read_register(this, ABOV_ABOV_WHOAMI_REG, &chip_id);
 return snprintf(buf, 16, "0x%x\n", chip_id);
}
static ssize_t sar_id_store(struct class *class,
  struct class_attribute *attr,
  const char *buf, size_t count)
{
   LOG_INFO("%s: abov_capsens_chip_id\n", __func__);
   return count;
}

static CLASS_ATTR_RW(sar_id);
//-Bug591052 tangsha.wt, MOD,20201012, add sar sensor chip id to factory interface
...
//+Bug591052 tangsha.wt, MOD,20201012, add sar sensor chip id to factory interface
  ret = class_create_file(&capsense_class, &class_attr_sar_id);
  if (ret < 0) {
   LOG_DBG("Create sarid file failed (%d)\n", ret);
   return ret;
  }
//-Bug591052 tangsha.wt, MOD,20201012, add sar sensor chip id to factory interface
