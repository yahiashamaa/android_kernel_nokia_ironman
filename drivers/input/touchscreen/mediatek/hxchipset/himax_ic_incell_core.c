...
static void himax_mcu_read_FW_ver(void)
{
	...
	I("Project ID = %s\n", ic_data->vendor_proj_info);
        ...
        snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"HX83102-D,VND:LIDE,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
...
int hx_parse_bin_cfg_data(const struct firmware *fw_entry, struct zf_info *zf_info_arr, uint8_t *FW_buf, uint8_t *sram_min, uint8_t *ovl_sidx)
{
	...
	memset(FW_buf, 0x00, ((sizeof(uint8_t) * cfg_sz) > FW_BIN_16K_SZ) ? FW_BIN_16K_SZ: (sizeof(uint8_t) * cfg_sz));
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	i_CTPM_firmware_name = "Himax_firmware_lide.bin";
