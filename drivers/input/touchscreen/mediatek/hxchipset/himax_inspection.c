...
static int hx_turn_on_mp_func(int on)
{
	...
	himax_in_parse_assign_cmd(addr_turn_on_mp_chk, tmp_addr, sizeof(tmp_addr));
	if (on) {
		I("%s : Turn on!\n", __func__);
			if (strcmp(HX_83102D_SERIES_PWON, private_ts->chip_name) == 0) {
				I("%s: need to enter Mp mode!\n", __func__);
				himax_in_parse_assign_cmd(addr_turn_on_mp, tmp_data, sizeof(tmp_data));
				do {
					g_core_fp.fp_register_write(tmp_addr, 4, tmp_data, 0);
...
static uint32_t mpTestFunc(uint8_t checktype, uint32_t datalen)
{
	...
	sub_len = snprintf(buf,  sizeof(buf), "\n%s%s\n",g_himax_inspection_mode[checktype], ": data as follow!\n");
	sub_len = strlen(buf);
	strncat(g_start_log+len, buf, sub_len);
	len += sub_len;
	...
	sub_len = strlen(buf);
	if((256 - len -1) > sub_len){
		strncat(g_start_log+len, buf, sub_len);
		len += sub_len;
	}else{
		E("No enough space print arraydata_min1.\n");
	}

	sub_len = snprintf(buf,  sizeof(buf), "  arraydata_min2 = %d,", arraydata_min2);
	sub_len = strlen(buf);
	if((256 - len -1 ) > sub_len){
		strncat(g_start_log+len, buf, sub_len);
		len += sub_len;
	}else{
		E("No enough space print arraydata_min2.\n");
	}

	sub_len = snprintf(buf,  sizeof(buf), "  arraydata_min3 = %d,", arraydata_min3);
	sub_len = strlen(buf);
	if((256 - len -1) > sub_len){
		strncat(g_start_log+len, buf, sub_len);
		len += sub_len;
	}else{
		E("No enough space print arraydata_min3.\n");
	}
	...
	if((256 - len -1) > sub_len){
		...
		E("No enough space print arraydata_max1.\n");
	}

	sub_len = snprintf(buf,  sizeof(buf), "  arraydata_max2 = %d,", arraydata_max2);
	sub_len = strlen(buf);
	if((256 - len -1) > sub_len){
		strncat(g_start_log+len, buf, sub_len);
		len += sub_len;
	}else{
		E("No enough space print arraydata_max2.\n");
	}

	sub_len = snprintf(buf,  sizeof(buf),"  arraydata_max3 = %d\n", arraydata_max3);
	sub_len = strlen(buf);
	if((256 - len -1) > sub_len){
		strncat(g_start_log+len, buf, sub_len);
		len += sub_len;
	}else{
		E("No enough space print arraydata_max3.\n");
	}
...
static int himax_self_test_data_init(void)
{
	...
	if(lcm_name == 0)
			...
			file_name_2 = "hx_criteria_lide.csv";
...
static int himax_chip_self_test(void)
{
	...
	uint8_t  *normalfw = NULL;
	uint8_t *mpapfw = NULL;

	if (lcm_name == 0) {
		 normalfw= "Himax_firmware.bin";
		 mpapfw= "Himax_mpfw.bin";
	} else {
		normalfw = "Himax_firmware_lide.bin";
		mpapfw = "Himax_mpfw_lide.bin";
	}
