...
void fts_gesture_recovery(struct fts_ts_data *ts_data)
{
    if (ts_data->gesture_mode && ts_data->suspended) {
        ...
        fts_ft8006p_boe_write_reg(0xD1, 0xFF);
        fts_ft8006p_boe_write_reg(0xD2, 0xFF);
        fts_ft8006p_boe_write_reg(0xD5, 0xFF);
        fts_ft8006p_boe_write_reg(0xD6, 0xFF);
        fts_ft8006p_boe_write_reg(0xD7, 0xFF);
        fts_ft8006p_boe_write_reg(0xD8, 0xFF);
        fts_ft8006p_boe_write_reg(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        fts_ft8006p_boe_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        fts_ft8006p_boe_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int fts_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        fts_ft8006p_boe_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        fts_ft8006p_boe_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
