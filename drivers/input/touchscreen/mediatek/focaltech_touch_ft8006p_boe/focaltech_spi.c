...
{
    int ret = 0;
    struct spi_device *spi = fts_ft8006p_boe_data->spi;
...
static int fts_wait_idle(void)
{
    ...
    u8 *txbuf = fts_ft8006p_boe_data->bus_tx_buf;
    u8 *rxbuf = fts_ft8006p_boe_data->bus_rx_buf;
    u32 txlen = 0;
...
static int fts_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    if (ts_data->fw_is_running && data && datalen) {
        ...
        fts_cmdpkg_wirte(WRITE_CMD, tmpcmd, cmdlen);
    }
...
int fts_ft8006p_boe_write(u8 *writebuf, u32 writelen)
{
    ...
    if (1 == writelen) {
        ...
        cmd = writebuf;
        cmdlen = 1;
        if (!fts_ft8006p_boe_data->fw_is_running) {
...
{
    return fts_ft8006p_boe_read(&addr, 1, value, 1);
}
