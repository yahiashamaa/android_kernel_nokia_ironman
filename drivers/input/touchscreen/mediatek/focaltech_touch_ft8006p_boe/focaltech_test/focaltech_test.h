...
#define FTS_SAVE_OPENSHORT_TEST_DATA_FILE_PATH  "/data/vendor/fac_sources/" 
...
*****************************************************************************/
extern struct test_funcs fts_ft8006p_boe_test_func;

extern struct fts_test *fts_ft8006p_boe_fts_ftest;
...
bool fts_ft8006p_boe_get_fw_wp(u8 wp_channel_select, enum wp_type water_proof_type);
...
bool fts_ft8006p_boe_compare_mc_sc(bool, bool, int *, int *, int *);
...
#define fts_ft8006p_boe_malloc_r(p, size) do {\
    if (NULL == p) {\
        p = fts_ft8006p_boe_malloc(size);\
        if (NULL == p) {\
            return -ENOMEM;\
        }\
    }\
...
#define FTS_TEST_SAVE_INFO(fmt, args...) do { \
    if (fts_ft8006p_boe_fts_ftest->testresult) { \
        fts_ft8006p_boe_fts_ftest->testresult_len += snprintf( \
        fts_ft8006p_boe_fts_ftest->testresult + fts_ft8006p_boe_fts_ftest->testresult_len, \
        TXT_BUFFER_LEN, \
        fmt, ##args);\
    } \
} while (0)
...
#define FTS_TEST_SAVE_ERR(fmt, args...)  do { \
    if (fts_ft8006p_boe_fts_ftest->testresult && (fts_ft8006p_boe_fts_ftest->testresult_len < TXT_BUFFER_LEN)) { \
        fts_ft8006p_boe_fts_ftest->testresult_len += snprintf( \
        fts_ft8006p_boe_fts_ftest->testresult + fts_ft8006p_boe_fts_ftest->testresult_len, \
        TXT_BUFFER_LEN, \
        fmt, ##args);\
