...
*****************************************************************************/
struct ini_ic_type fts_ft8006p_boe_ic_types[] = {
    {"FT5X46",  0x54000002},
    {"FT5X46i", 0x54010002},
    {"FT5526",  0x54020002},
    {"FT3X17",  0x54030002},
...
static int fts_test_get_ini_via_request_firmware(struct ini_data *ini, char *fwname)
{
    ...
    struct device *dev = &fts_ft8006p_boe_data->input_dev->dev;
...
static int ini_get_key(char *section_name, char *key_name, char *value)
{
    int i = 0;
    int j = 0;
    struct ini_data *ini = &fts_ft8006p_boe_fts_ftest->ini;
    struct ini_section *section;
    struct ini_keyword *keyword;
    int key_len = 0;
    int log_level = fts_ft8006p_boe_fts_ftest->ts_data->log_level;
...
int fts_ft8006p_boe_get_keyword_value(char *section, char *name, int *value)
{
    int ret = 0;
...
static int get_basic_threshold(char name[][MAX_KEYWORD_NAME_LEN], int length, int *val)
{
    ...
    if (length > fts_ft8006p_boe_fts_ftest->basic_thr_count) {
...
static int init_node_valid(void)
{
    ...
    if ((tdata->func->hwtype == IC_HW_INCELL) || (tdata->func->hwtype == IC_HW_MC_SC)) {
        for (cnt = 0; cnt < node_num; cnt++) {
            ...
            fts_ft8006p_boe_get_keyword_value("INVALID_NODE", str, &tdata->node_valid[cnt]);
        }
    }
    ...
    if (tdata->func->hwtype == IC_HW_MC_SC) {
        ...
        for (cnt = 0; cnt < node_num; cnt++) {
            ...
            fts_ft8006p_boe_get_keyword_value("INVALID_NODES", str, &tdata->node_valid_sc[cnt]);
        }
    }

    fts_ft8006p_boe_print_buffer(tdata->node_valid, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(tdata->node_valid_sc, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    return 0;
}
...
static int get_test_item_incell(void)
{
    ...
    fts_ft8006p_boe_fts_ftest->ic.incell.u.tmp = item_val;
    return 0;
}
...
static void print_thr_incell(void)
{
    ...
    fts_ft8006p_boe_print_buffer(thr->rawdata_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->rawdata_max, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->cb_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->cb_max, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->rawdata2_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->rawdata2_max, tdata->node.node_num, tdata->node.rx_num);
}
...
static int get_test_item_mc_sc(void)
{
    ...
    fts_ft8006p_boe_fts_ftest->ic.mc_sc.u.tmp = item_val;
    FTS_TEST_INFO("test item:0x%x in ini", fts_ft8006p_boe_fts_ftest->ic.mc_sc.u.tmp);
    return 0;
}
...
static void print_thr_mc_sc(void)
{
    ...
    fts_ft8006p_boe_print_buffer(thr->rawdata_h_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->rawdata_h_max, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->rawdata_l_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->rawdata_l_max, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_cb_off_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_cb_off_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_cb_on_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_cb_on_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_rawdata_off_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_rawdata_off_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_rawdata_on_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->scap_rawdata_on_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->panel_differ_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->panel_differ_max, tdata->node.node_num, tdata->node.rx_num);
}
...
static int get_test_item_sc(void)
{
    ...
    fts_ft8006p_boe_fts_ftest->ic.sc.u.tmp = item_val;
    return 0;
}
...
static void print_thr_sc(void)
{
    ...
    fts_ft8006p_boe_print_buffer(thr->cb_min, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->cb_max, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->dcb_sort, tdata->node.node_num, tdata->node.rx_num);
    fts_ft8006p_boe_print_buffer(thr->dcb_base, tdata->node.node_num, tdata->node.rx_num);
}
...
static u32 ini_get_ic_code(char *ic_name)
{
    ...
    int fts_ft8006p_boe_ic_types_len = 0;

    ini_icname_len = strlen(ic_name);
    type_size = sizeof(fts_ft8006p_boe_ic_types) / sizeof(fts_ft8006p_boe_ic_types[0]);
    for (i = 0; i < type_size; i++) {
        fts_ft8006p_boe_ic_types_len = strlen(ic_name);
        if (ini_icname_len == fts_ft8006p_boe_ic_types_len) {
            if (0 == strncmp(ic_name, fts_ft8006p_boe_ic_types[i].ic_name, fts_ft8006p_boe_ic_types_len))
                return fts_ft8006p_boe_ic_types[i].ic_type;
        }
    }
...
/*
 * fts_ft8006p_boe_test_get_testparam_from_ini - get test parameters from ini
 *
 * read, parse the configuration file, initialize the test variable
 *
 * return 0 if succuss, else errro code
 */
int fts_ft8006p_boe_test_get_testparam_from_ini(char *config_name)
{
    int ret = 0;
    struct ini_data *ini = &fts_ft8006p_boe_fts_ftest->ini;
