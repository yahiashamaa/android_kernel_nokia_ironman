...
static int fts_enter_proximity_mode(int mode)
{
    ...
    ret = fts_ft8006p_boe_write_reg(buf_addr, buf_value);
    if (ret < 0) {
...
static int ps_enable_nodata(int en)
{
    ...
    /* Enable proximity */
    mutex_lock(&fts_ft8006p_boe_data->input_dev->mutex);
    err = fts_enter_proximity_mode(en);
    mutex_unlock(&fts_ft8006p_boe_data->input_dev->mutex);
    return err;
}
...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    fts_ft8006p_boe_read_reg(FTS_REG_FACE_DEC_MODE_STATUS, &regvalue);
