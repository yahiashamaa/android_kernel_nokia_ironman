...
*            1. Add LCD_ESD_PATCH to control fts_ft8006p_boe_idc_esdcheck_lcderror
*****************************************************************************/
...
int fts_ft8006p_boe_idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    int ret = 0;
    u8 val = 0;
    ...
    if ( (tp_need_recovery == 1) && (lcd_need_reset == 0) ) {
        ...
        fts_ft8006p_boe_tp_state_recovery(ts_data);
    }

    ret = fts_ft8006p_boe_read_reg(FTS_REG_ESD_SATURATE, &val);
    if ( ret < 0) {
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret = fts_ft8006p_boe_read(&reg_addr, 1, &reg_value, 1);
        if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_ft8006p_boe_read_reg(reg_addr, &reg_value);
    if ( ret < 0 ) {
        fts_esdcheck_data.nack_cnt++;
...
#if LCD_ESD_PATCH
    fts_ft8006p_boe_idc_esdcheck_lcderror(ts_data);
#endif
...
static void esdcheck_func(struct work_struct *work)
{
	...
	fts_ft8006p_boe_read_reg(0xc8,&val_1);
	 if (0x1 == val_1)
	 {
		lcm_power_disable();
		msleep(6);
		lcm_power_enable();
		fts_ft8006p_boe_write_reg(0xc9,0x1);
	 }
...
/*****************************************************************************
*  Name: fts_ft8006p_boe_esdcheck_proc_busy
...
/*****************************************************************************
*  Name: fts_ft8006p_boe_esdcheck_switch
...
int fts_ft8006p_boe_create_esd_sysfs(struct device *dev)
{
    int ret = 0;

    ret = sysfs_create_group(&dev->kobj, &fts_esd_group);
    if ( ret != 0) {
        FTS_ERROR("fts_ft8006p_boe_create_esd_sysfs(sysfs) create fail");
...
int fts_ft8006p_boe_esdcheck_init(struct fts_ts_data *ts_data)
{
    ...
    fts_esdcheck_data.intr_cnt = 0;
    fts_ft8006p_boe_esdcheck_switch(ENABLE);
    fts_ft8006p_boe_create_esd_sysfs(ts_data->dev);
    FTS_FUNC_EXIT();
    return 0;
}
