...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "fts_ts_ft8006p_boe"
...
*****************************************************************************/
struct fts_ts_data *fts_ft8006p_boe_data;
...
/*****************************************************************************
*  Name: fts_ft8006p_boe_wait_tp_to_valid
...
*****************************************************************************/
int fts_ft8006p_boe_wait_tp_to_valid(void)
{
    int ret = 0;
    int cnt = 0;
    u8 idh = 0;
    u8 idl = 0;
    u8 chip_idh = fts_ft8006p_boe_data->ic_info.ids.chip_idh;
    u8 chip_idl = fts_ft8006p_boe_data->ic_info.ids.chip_idl;

    do {
        ret = fts_ft8006p_boe_read_reg(FTS_REG_CHIP_ID, &idh);
        ret = fts_ft8006p_boe_read_reg(FTS_REG_CHIP_ID2, &idl);
...
/*****************************************************************************
*  Name: fts_ft8006p_boe_tp_state_recovery
*  Brief: Need execute this function when reset
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_ft8006p_boe_tp_state_recovery(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    /* wait tp stable */
    fts_ft8006p_boe_wait_tp_to_valid();
    /* recover TP charger state 0x8B */
    /* recover TP glove state 0xC0 */
    /* recover TP cover state 0xC1 */
    fts_ft8006p_boe_ex_mode_recovery(ts_data);
#if FTS_PSENSOR_EN
...
int fts_ft8006p_boe_reset_proc(int hdelayms)
{
    FTS_DEBUG("tp reset");
    tpd_gpio_output(fts_ft8006p_boe_data->pdata->reset_gpio, 0);
    msleep(1);
    tpd_gpio_output(fts_ft8006p_boe_data->pdata->reset_gpio, 1);
    if (hdelayms) {
        msleep(hdelayms);
    }
...
void fts_ft8006p_boe_irq_disable(void)
{
    unsigned long irqflags;

    FTS_FUNC_ENTER();
    spin_lock_irqsave(&fts_ft8006p_boe_data->irq_lock, irqflags);

    if (!fts_ft8006p_boe_data->irq_disabled) {
        disable_irq_nosync(fts_ft8006p_boe_data->irq);
        fts_ft8006p_boe_data->irq_disabled = true;
    }

    spin_unlock_irqrestore(&fts_ft8006p_boe_data->irq_lock, irqflags);
    FTS_FUNC_EXIT();
}

void fts_ft8006p_boe_irq_enable(void)
{
    ...
    if (fts_ft8006p_boe_data->irq_disabled) {
        enable_irq(fts_ft8006p_boe_data->irq);
        fts_ft8006p_boe_data->irq_disabled = false;
    }
...
void fts_ft8006p_boe_hid2std(void)
{
    int ret = 0;
    u8 buf[3] = {0xEB, 0xAA, 0x09};

    ret = fts_ft8006p_boe_write(buf, 3);
    if (ret < 0) {
        FTS_ERROR("hid2std cmd write fail");
    } else {
        msleep(10);
        buf[0] = buf[1] = buf[2] = 0;
        ret = fts_ft8006p_boe_read(NULL, 0, buf, 3);
        if (ret < 0) {
...
static int fts_ft8006p_boe_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
    int ret = 0;
    u8 chip_id[2] = { 0 };
    u8 id_cmd[4] = { 0 };
    ...
    ret = fts_ft8006p_boe_write(id_cmd, 2);
    if (ret < 0) {
    ...
    ret = fts_ft8006p_boe_read(id_cmd, id_cmd_len, chip_id, 2);
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    for (cnt = 0; cnt < 3; cnt++) {
        fts_ft8006p_boe_reset_proc(0);
        mdelay(FTS_CMD_START_DELAY);

        ret = fts_ft8006p_boe_read_bootid(ts_data, &chip_id[0]);
        if (ret <  0) {
...
void fts_ft8006p_boe_release_all_finger(void)
{
    struct input_dev *input_dev = fts_ft8006p_boe_data->input_dev;
#if FTS_MT_PROTOCOL_B_EN
    u32 finger_count = 0;
    u32 max_touches = fts_ft8006p_boe_data->pdata->max_touch_number;
#endif

    FTS_FUNC_ENTER();
    mutex_lock(&fts_ft8006p_boe_data->report_mutex);
#if FTS_MT_PROTOCOL_B_EN
    ...
    fts_ft8006p_boe_data->touchs = 0;
    fts_ft8006p_boe_data->key_state = 0;
    mutex_unlock(&fts_ft8006p_boe_data->report_mutex);
    FTS_FUNC_EXIT();
}
...
static int fts_ft8006p_boe_read_touchdata(struct fts_ts_data *data)
{
    int ret = 0;
    ...
    ret = fts_ft8006p_boe_read(buf, 1, buf + 1, data->pnt_buf_size - 1);
    if (((0xEF == buf[2]) && (0xEF == buf[3]) && (0xEF == buf[4]))
        || ((ret < 0) && (0xEF == buf[1]))) {
        /* check if need recovery fw */
        fts_ft8006p_boe_fw_recovery();
...
static int fts_ft8006p_boe_read_parse_touchdata(struct fts_ts_data *data)
{
    int ret = 0;
    int i = 0;
    u8 pointid = 0;
    ...
    ret = fts_ft8006p_boe_read_touchdata(data);
    if (ret) {
        return ret;
    }
    ...
    if (data->ic_info.is_incell) {
        if ((data->point_num == 0x0F) && (buf[2] == 0xFF) && (buf[3] == 0xFF)
            ...
            fts_ft8006p_boe_release_all_finger();
            fts_ft8006p_boe_tp_state_recovery(data);
            return -EIO;
        }
    }
...
static void fts_irq_read_report(void)
{
...
#if FTS_ESDCHECK_EN
    fts_ft8006p_boe_esdcheck_set_intr(1);
#endif
    ...
    ret = fts_ft8006p_boe_read_parse_touchdata(ts_data);
    if (ret == 0) {
...
#if FTS_ESDCHECK_EN
    fts_ft8006p_boe_esdcheck_set_intr(0);
#endif
}
...
#if FTS_PSENSOR_EN
        if (fts_proximity_readdata(fts_ft8006p_boe_data) == 0)
            continue;
#endif
...
#ifdef FTS_CONFIG_TOUCHSCREEN_LOCKDOWN_INFO
...
#define  CTP_OPEN_SHORT_TEST_INI_NAME	"T99621_Focal_txd_boe.ini"
extern int fts_ft8006p_boe_open_short_test(char *ini_file_name);
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
	...
	*ppos += count;
	...
	if (ret < 0) {
		...
		ret = fts_ft8006p_boe_open_short_test(fwname);
		if (ret == 1) {
			result = 1;
...
void fts_ft8006p_boe_create_ctp_proc(void)
{
   //----------------------------------------
	...
	printk("openshort fts_ft8006p_boe_create_ctp_proc \n");
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_ft8006p_boe_bus_init(ts_data);
    if (ret) {
...
#if (!FTS_CHIP_IDC)
    fts_ft8006p_boe_reset_proc(200);
#endif
...
#if FTS_APK_NODE_EN
    ret = fts_ft8006p_boe_create_apk_debug_channel(ts_data);
    if (ret) {
    ...
    ret = fts_ft8006p_boe_create_sysfs(ts_data);
    if (ret) {
    ...
    ret = fts_ft8006p_boe_ex_mode_init(ts_data);
    if (ret) {
...
#if FTS_TEST_EN
    ret = fts_ft8006p_boe_test_init(ts_data);
    if (ret) {
...
#if FTS_ESDCHECK_EN
    ret = fts_ft8006p_boe_esdcheck_init(ts_data);
    if (ret) {
...
#if OPEN_SHORT_PROC
	 fts_ft8006p_boe_create_ctp_proc();
#endif
    ...
    ret = fts_ft8006p_boe_fwupg_init(ts_data);
    if (ret) {
...
#if FTS_APK_NODE_EN
    fts_ft8006p_boe_release_apk_debug_channel(ts_data);
#endif
#if FTS_SYSFS_NODE_EN
    fts_ft8006p_boe_remove_sysfs(ts_data);
#endif
    fts_ft8006p_boe_ex_mode_exit(ts_data);

    fts_ft8006p_boe_fwupg_exit(ts_data);

#if FTS_TEST_EN
    fts_ft8006p_boe_test_exit(ts_data);
#endif

#if FTS_ESDCHECK_EN
    fts_ft8006p_boe_esdcheck_exit(ts_data);
#endif

#if FTS_GESTURE_EN
    fts_gesture_exit(ts_data);
#endif
    fts_ft8006p_boe_bus_exit(ts_data);
...
void fts_ft8006p_boe_spi_select_active(struct fts_ts_data *data)
{
    FTS_FUNC_ENTER();
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	FTS_ERROR(" spi->max_speed_hz == %d.", spi->max_speed_hz);
    ...
    if (!ts_data) {
        FTS_ERROR("allocate memory for fts_ft8006p_boe_data fail");
        return -ENOMEM;
    }
    ...
    if (ret == 0) {
        fts_ft8006p_boe_spi_select_active(ts_data);
    }

    fts_ft8006p_boe_data = ts_data;
    ts_data->spi = spi;
    ts_data->dev = &spi->dev;
    ts_data->log_level = 1;
...
static const struct of_device_id fts_dt_match[] = {
    {.compatible = "focaltech_ft8006p_boe,fts", },
    {},
};
...
static void tpd_suspend(struct device *dev)
{
...
#if FTS_ESDCHECK_EN
    fts_ft8006p_boe_esdcheck_suspend();
#endif
	...
	if (ts_data->gesture_mode) {
        ...
        ret = fts_ft8006p_boe_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP);
        if (ret < 0)
...
#if FTS_ESDCHECK_EN
	fts_ft8006p_boe_esdcheck_resume();
#endif
