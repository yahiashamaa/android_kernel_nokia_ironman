    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_WRITE_REGISTER:
        ret = fts_ft8006p_write_reg(writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_WRITE_DATA:
        writelen = buflen - 1;
        ret = fts_ft8006p_write(writebuf + 1, writelen);
        if (ret < 0) {
    ...
    case PROC_ENTER_TEST_ENVIRONMENT:
        ...
        if (0 == writebuf[1]) {
            fts_ft8006p_enter_test_environment(0);
        } else {
            fts_ft8006p_enter_test_environment(1);
        }
        break;
    ...
    struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = fts_ft8006p_read_reg(proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        ret = fts_ft8006p_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
int fts_ft8006p_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
void fts_ft8006p_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
...
{
    ssize_t count = 0;
    struct irq_desc *desc = irq_to_desc(fts_ft8006p_data->irq);
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&input_dev->mutex);
    if (true == fts_ft8006p_data->fw_is_running) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_ft8006p_esdcheck_proc_busy(1);
#endif
    fts_ft8006p_read_reg(FTS_REG_FW_VER, &fwver);
#if FTS_ESDCHECK_EN
    fts_ft8006p_esdcheck_proc_busy(0);
#endif
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts_ft8006p_read_reg(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts_ft8006p_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts_ft8006p_write_reg(reg, val);
            } else {
                rw_op.res = fts_ft8006p_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
/* fts_ft8006p_upgrade_bin interface */
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_ft8006p_upgrade_bin(fwname, 0);
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_ft8006p_upgrade_bin(fwname, 1);
    mutex_unlock(&input_dev->mutex);
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < fts_ft8006p_data->pnt_buf_size; i++) {
        count += snprintf(buf + count, PAGE_SIZE, "%02x ", fts_ft8006p_data->point_buf[i]);
    }
    count += snprintf(buf + count, PAGE_SIZE, "\n");
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);
    FTS_DEBUG("log level:%d->%d", fts_ft8006p_data->log_level, value);
    fts_ft8006p_data->log_level = value;
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
...
/*  upgrade from fw bin file   example:echo "*.bin" > fts_ft8006p_upgrade_bin */
...
int fts_ft8006p_create_sysfs(struct fts_ts_data *ts_data)
{
    int ret = 0;
...
int fts_ft8006p_remove_sysfs(struct fts_ts_data *ts_data)
{
    sysfs_remove_group(&ts_data->dev->kobj, &fts_attribute_group);
    return 0;
}
