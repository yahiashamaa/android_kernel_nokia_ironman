...
*****************************************************************************/
u8 fts_ft8006p_fw_file[] = {
#include FTS_UPGRADE_FW_FILE
};

u8 fts_ft8006p_fw_file2[] = {
#include FTS_UPGRADE_FW2_FILE
};

u8 fts_ft8006p_fw_file3[] = {
#include FTS_UPGRADE_FW3_FILE
};

struct upgrade_module fts_ft8006p_module_list[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, fts_ft8006p_fw_file, sizeof(fts_ft8006p_fw_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, fts_ft8006p_fw_file2, sizeof(fts_ft8006p_fw_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, fts_ft8006p_fw_file3, sizeof(fts_ft8006p_fw_file3)},
};

struct fts_upgrade *fts_ft8006p_fwupgrade;

 //+ add by songbinbo.wt for tp info in factory mode  20190413
#define FT8006P_IC_CHIP_ID   0x86
#define FT8006P_TRULY_VENDOR_ID    0x5a
...
static int fts_check_bootid(void)
{
    ...
    u8 cmd = 0;
    u8 id[2] = { 0 };
    struct ft_chip_t *chip_id = &fts_ft8006p_data->ic_info.ids;

    cmd = FTS_CMD_READ_ID;
    ret = fts_ft8006p_read(&cmd, 1, id, 2);
    if (ret < 0) {
...
#if FTS_ESDCHECK_EN
    fts_ft8006p_esdcheck_proc_busy(1);
#endif

    fts_ft8006p_read_reg(FTS_REG_VENDOR_ID, &tp_vendor_id);
    fts_ft8006p_read_reg( FTS_REG_CHIP_ID, &tp_ic_type);
    fts_ft8006p_read_reg(FTS_REG_FW_VER, &firmware_version_from_ic);

#if FTS_ESDCHECK_EN
    fts_ft8006p_esdcheck_proc_busy(0);
#endif
...
static void fts_hardwareinfo_set(void)
{
	...
	if(tp_vendor_id== FT8006P_TRULY_VENDOR_ID)
	{
		snprintf(vendor_id,HARDWARE_MAX_ITEM_LONGTH,"TRULY");
	}else {
...
static bool fts_check_fast_download(void)
{
    ...
    if (!((0x87 == fts_ft8006p_data->ic_info.ids.rom_idh)
          && (0x19 == fts_ft8006p_data->ic_info.ids.rom_idl))) {
        return true;
    }

    ret = fts_ft8006p_read_reg(0xdb, &value);
    if (ret < 0) {
        FTS_ERROR("read 0xdb fail");
        goto read_err;
    }

    ret = fts_ft8006p_read(cmd, 6, value2, 2);
    if (ret < 0) {
...
static int fts_dpram_write(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_ft8006p_write(&cmd[0], FTS_CMD_WRITE_LEN + packet_len);
        if (ret < 0) {
...
static int fts_ecc_cal_tp(u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
    ...
    ret = fts_ft8006p_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
        ret = fts_ft8006p_read(cmd, 1, value, 1);
        if (ret < 0) {
    ...
    ret = fts_ft8006p_read(cmd, 1, value, 2);
    if (ret < 0) {
...
static int fts_fw_write_start(const u8 *buf, u32 len, bool need_reset)
{
    ...
    fts_ft8006p_data->fw_is_running = false;
    ...
    fts_ft8006p_data->fw_is_running = true;
...
static int fts_fw_download(const u8 *buf, u32 len, bool need_reset)
{
    ...
    ret = 0;
err_fw_download:
    fts_ft8006p_irq_enable();
    ts_data->fw_loading = 0;

    return ret;
}

int fts_ft8006p_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;
...
int fts_ft8006p_upgrade_bin(char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
    struct fts_upgrade *upg = fts_ft8006p_fwupgrade;
    ...
    ret = fts_ft8006p_read_file(fw_name, &fw_file_buf);
...
int fts_ft8006p_enter_test_environment(bool test_state)
{
    int ret = 0;
    u8 detach_flag = 0;
    struct fts_upgrade *upg = fts_ft8006p_fwupgrade;
    ...
    if (fts_ft8006p_data->fw_loading) {
        FTS_INFO("fw is loading, not download again");
        return -EINVAL;
    }
    ...
    msleep(50);
    ret = fts_ft8006p_read_reg(FTS_REG_FACTORY_MODE_DETACH_FLAG, &detach_flag);
    FTS_INFO("regb4:0x%02x", detach_flag);

    return 0;
}

int fts_ft8006p_fw_resume(void)
{
    int ret = 0;
    struct fts_upgrade *upg = fts_ft8006p_fwupgrade;
...
int fts_ft8006p_fw_recovery(void)
{
    int ret = 0;
    u8 boot_state = 0;
    u8 chip_id = 0;
    ...
    ret = fts_ft8006p_read_reg(0xD0, &boot_state);
    if (ret < 0) {
    ...
    ret = fts_ft8006p_fw_resume();
    if (ret < 0) {
        FTS_ERROR("fts_ft8006p_fw_resume fail");
        return ret;
    }
    msleep(10);
    ret = fts_ft8006p_read_reg(FTS_REG_CHIP_ID, &chip_id);
    FTS_INFO("read chip id:0x%02x", chip_id);

    fts_ft8006p_tp_state_recovery(upg->ts_data);
...
{
    int i = 0;
    struct upgrade_module *info = &fts_ft8006p_module_list[0];
    ...
    if (FTS_GET_MODULE_NUM > 1) {
        ...
        for (i = 0; i < FTS_GET_MODULE_NUM; i++) {
            info = &fts_ft8006p_module_list[i];
...
int fts_ft8006p_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    fts_ft8006p_fwupgrade = (struct fts_upgrade *)kzalloc(sizeof(*fts_ft8006p_fwupgrade), GFP_KERNEL);
    if (NULL == fts_ft8006p_fwupgrade) {
    ...
    fts_ft8006p_fwupgrade->ts_data = ts_data;
...
int fts_ft8006p_fwupg_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (fts_ft8006p_fwupgrade) {
        if (fts_ft8006p_fwupgrade->fw_from_request) {
            vfree(fts_ft8006p_fwupgrade->fw);
            fts_ft8006p_fwupgrade->fw = NULL;
        }

        kfree(fts_ft8006p_fwupgrade);
        fts_ft8006p_fwupgrade = NULL;
    }
    FTS_FUNC_EXIT();
    return 0;
}
