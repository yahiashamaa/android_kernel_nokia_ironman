...
static int ft8006p_short_test(struct fts_test *tdata, bool *test_result)
{
    ...
    ret = fts_ft8006p_short_get_adcdata_incell(TEST_RETVAL_AA, ch_num, byte_num, adcdata);
    if (ret < 0) {
    ...
    /* show data in txt */
    fts_ft8006p_show_data(adcdata, false);

    /* compare */
    min = thr->basic.short_res_min;
    max = TEST_SHORT_RES_MAX;
    tmp_result = fts_ft8006p_compare_data(adcdata, min, max, min, max, false);

    ret = 0;
test_err:
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_SHORT_TEST_STATE, 0x03);
    if (ret < 0) {
    ...
    /* save data */
    fts_ft8006p_test_save_data("Short Circuit Test", CODE2_SHORT_TEST,
...
static int ft8006p_open_test(struct fts_test *tdata, bool *test_result)
{
    ...
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_OPEN_START, 0x01);
    if (ret < 0) {
    ...
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        ...
        ret = fts_ft8006p_test_read_reg(FACTORY_REG_OPEN_STATE, &state);
    ...
    ret = fts_ft8006p_get_cb_incell(0, byte_num, opendata);
    if (ret) {
    ...
    /* show data in txt */
    fts_ft8006p_show_data(opendata, false);

    /* compare */
    min = thr->basic.open_cb_min;
    max = TEST_OPEN_MAX_VALUE;
    tmp_result = fts_ft8006p_compare_data(opendata, min, max, 0, 0, false);

restore_reg:
    /* auto clb */
    ret = fts_ft8006p_chip_clb();
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("auto clb fail\n");
    }

test_err:
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_OPEN_STATE, 0x03);
    if (ret < 0) {
    ...
    /* save */
    fts_ft8006p_test_save_data("Open Test", CODE_OPEN_TEST,
...
static int ft8006p_cb_test(struct fts_test *tdata, bool *test_result)
{
    ...
    /* cb test enable */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_CB_TEST_EN, 0x01);
    if (ret < 0) {
    ...
    byte_num = tdata->node.node_num;
    ret = fts_ft8006p_get_cb_incell(0, byte_num, cbdata);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("get cb fail\n");
        goto test_err;
    }
    ...
    /* show data in txt */
    fts_ft8006p_show_data(cbdata, false);

    /* compare */
    tmp_result = fts_ft8006p_compare_array(cbdata, thr->cb_min, thr->cb_max, false);

test_err:
    /* cb test disable */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_CB_TEST_EN, 0x00);
    if (ret < 0) {
    ...
    /* save */
    fts_ft8006p_test_save_data("CB Test", CODE_CB_TEST,
...
static int ft8006p_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    ...
    /* rawdata test enable */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_RAWDATA_TEST_EN, 0x01);
    if (ret < 0) {
    ...
    /* show dat in txt */
    fts_ft8006p_show_data(rawdata, false);

    /* compare */
    tmp_result = fts_ft8006p_compare_array(rawdata,
                               thr->rawdata_min,
                               thr->rawdata_max,
                               false);
    ret = 0;
test_err:
    /* rawdata test disble */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_RAWDATA_TEST_EN, 0x00);
    if (ret < 0) {
    ...
    /* save */
    fts_ft8006p_test_save_data("RawData Test", CODE_RAWDATA_TEST,
...
static int ft8006p_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
    ...
    ret = fts_ft8006p_test_read_reg(FACTORY_REG_LCD_NOISE_TTHR, &touch_thr);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read reg14 fail\n");
        goto test_err;
    }

    ret = fts_ft8006p_test_read_reg(FACTORY_REG_DATA_SELECT, &old_mode);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read reg06 fail\n");
        goto test_err;
    }

    ret =  fts_ft8006p_test_write_reg(FACTORY_REG_DATA_SELECT, 0x01);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write 1 to reg06 fail\n");
        goto restore_reg;
    }

    ret =  fts_ft8006p_test_write_reg(FACTORY_REG_LINE_ADDR, 0xAD);
    if (ret < 0) {
    ...
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_LCD_NOISE_FRAME, frame_num / 4);
    if (ret < 0) {
    ...
    /* start test */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_LCD_NOISE_START, 0x01);
    if (ret < 0) {
    ...
    /* check test status */
    fts_ft8006p_sys_delay(frame_num * FACTORY_TEST_DELAY / 2);
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        status = 0xFF;
        ret = fts_ft8006p_test_read_reg(FACTORY_REG_LCD_NOISE_STATE, &status);
    ...
    /* read lcdnoise */
    byte_num = tdata->node.node_num * 2;
    ret = fts_ft8006p_read_mass_data(FACTORY_REG_RAWDATA_ADDR, byte_num, lcdnoise);
    if (ret < 0) {
    ...
    /* show data in txt */
    fts_ft8006p_show_data(lcdnoise, false);

    /* compare */
    max = thr->basic.lcdnoise_coefficient * touch_thr * 32 / 100;
    FTS_TEST_DBG("touch thr:%d, max:%d", touch_thr, max);
    tmp_result = fts_ft8006p_compare_data(lcdnoise, 0, max, 0, 0, false);

restore_reg:
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_LCD_NOISE_START, 0x00);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write 0 to reg11 fail\n");
    }

    ret = fts_ft8006p_test_write_reg(FACTORY_REG_DATA_SELECT, old_mode);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore reg06 fail\n");
    }
test_err:
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_LCD_NOISE_STATE, 0x03);
    if (ret < 0) {
    ...
    /* save */
    fts_ft8006p_test_save_data("LCD Noise Test", CODE2_LCD_NOISE_TEST,
...
static int start_test_ft8006p(void)
{
    ...
    FTS_TEST_FUNC_ENTER();
    FTS_TEST_INFO("test item:0x%x", fts_ft8006p_ftest->ic.incell.u.tmp);
