...
#define FTS_INI_FILE_PATH               "/vendor/firmware/"    // add by songbinbo .wt for tp open short  20190801
#define FTS_SAVE_OPENSHORT_TEST_DATA_FILE_PATH  "/data/vendor/fac_sources/"  // add by songbinbo.wt for forcal tp open short  2019080106
...
extern struct fts_test *fts_ft8006p_ftest;
...
bool fts_ft8006p_get_fw_wp(u8 wp_channel_select, enum wp_type water_proof_type);
...
bool fts_ft8006p_compare_mc_sc(bool, bool, int *, int *, int *);
...
#define FTS_TEST_SAVE_INFO(fmt, args...) do { \
    if (fts_ft8006p_ftest->testresult) { \
        fts_ft8006p_ftest->testresult_len += snprintf( \
        fts_ft8006p_ftest->testresult + fts_ft8006p_ftest->testresult_len, \
        TXT_BUFFER_LEN, \
        fmt, ##args);\
    } \
} while (0)
...
#define FTS_TEST_SAVE_ERR(fmt, args...)  do { \
    if (fts_ft8006p_ftest->testresult && (fts_ft8006p_ftest->testresult_len < TXT_BUFFER_LEN)) { \
        fts_ft8006p_ftest->testresult_len += snprintf( \
        fts_ft8006p_ftest->testresult + fts_ft8006p_ftest->testresult_len, \
        TXT_BUFFER_LEN, \
        fmt, ##args);\
