...
*****************************************************************************/
struct fts_test *fts_ft8006p_ftest;

struct test_funcs *fts_ft8006p_test_func_list[] = {
    &test_func_ft8006p,
};
...
* functions body
*****************************************************************************/
void fts_ft8006p_sys_delay(int ms)
{
    msleep(ms);
}

int fts_ft8006p_focal_abs(int value)
{
    if (value < 0)
        value = 0 - value;
...
void *fts_ft8006p_malloc(size_t size)
{
    return kzalloc(size, GFP_KERNEL);
}

void fts_ft8006p_free_proc(void *p)
{
    return kfree(p);
}

void fts_ft8006p_print_buffer(int *buffer, int length, int line_num)
{
    int i = 0;
    int j = 0;
    int tmpline = 0;
    ...
    int tmplen = 0;
    int cnt = 0;
    struct fts_test *tdata = fts_ft8006p_ftest;
    ...
    u8 *cmd, int cmdlen, u8 *data, int datalen)
{
    ...
    ret = fts_ft8006p_read(cmd, cmdlen, data, datalen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
    ...
    ret = fts_ft8006p_write(writebuf, writelen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
int fts_ft8006p_test_read_reg(u8 addr, u8 *val)
{
    return fts_test_bus_read(&addr, 1, val, 1);
}

int fts_ft8006p_test_write_reg(u8 addr, u8 val)
{
    int ret;
    u8 cmd[2] = {0};
...
int fts_ft8006p_test_read(u8 addr, u8 *readbuf, int readlen)
{
    int ret = 0;
    int i = 0;
    int packet_length = 0;
...
int fts_ft8006p_test_write(u8 addr, u8 *writebuf, int writelen)
{
    int ret = 0;
    int i = 0;
    u8 *data = NULL;
    ...
    data = fts_ft8006p_malloc(BYTES_PER_TIME + 1);
    if (!data) {
 ...
 *******************************************************************/
int fts_ft8006p_enter_work_mode(void)
{
    int ret = 0;
    u8 mode = 0;
    int i = 0;
    ...
    ret = fts_ft8006p_test_read_reg(DEVIDE_MODE_ADDR, &mode);
    if ((ret >= 0) && (0x00 == mode))
        return 0;

    for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
        ret = fts_ft8006p_test_write_reg(DEVIDE_MODE_ADDR, 0x00);
        if (ret >= 0) {
            fts_ft8006p_sys_delay(FACTORY_TEST_DELAY);
            for (j = 0; j < 20; j++) {
                ret = fts_ft8006p_test_read_reg(DEVIDE_MODE_ADDR, &mode);
                if ((ret >= 0) && (0x00 == mode)) {
...
int fts_ft8006p_enter_factory_mode(void)
{
    int ret = 0;
    u8 mode = 0;
    int i = 0;
    ...
    for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
        ret = fts_ft8006p_test_write_reg(DEVIDE_MODE_ADDR, 0x40);
        if (ret >= 0) {
            fts_ft8006p_sys_delay(FACTORY_TEST_DELAY);
            for (j = 0; j < 20; j++) {
                ret = fts_ft8006p_test_read_reg(DEVIDE_MODE_ADDR, &mode);
...
/*
 * fts_ft8006p_read_mass_data - read rawdata/short test data
 ...
 */
int fts_ft8006p_read_mass_data(u8 addr, int byte_num, int *buf)
{
    int ret = 0;
    int i = 0;
    u8 *data = NULL;

    data = (u8 *)fts_ft8006p_malloc(byte_num * sizeof(u8));
    if (NULL == data) {
    ...
    ret = fts_ft8006p_test_read(addr, data, byte_num);
    if (ret < 0) {
...
int fts_ft8006p_short_get_adcdata_incell(u8 retval, u8 ch_num, int byte_num, int *adc_buf)
{
    int ret = 0;
    int times = 0;
    u8 short_state = 0;
    ...
    /* Start ADC sample */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_SHORT_TEST_EN, 0x01);
    if (ret) {
        FTS_TEST_SAVE_ERR("start short test fail\n");
        goto adc_err;
    }

    fts_ft8006p_sys_delay(ch_num * FACTORY_TEST_DELAY);
    for (times = 0; times < FACTORY_TEST_RETRY; times++) {
        ret = fts_ft8006p_test_read_reg(FACTORY_REG_SHORT_TEST_STATE, &short_state);
        ...
        fts_ft8006p_sys_delay(FACTORY_TEST_RETRY_DELAY);
    }
    ...
    ret = fts_ft8006p_read_mass_data(FACTORY_REG_SHORT_ADDR, byte_num, adc_buf);
    if (ret) {
...
/*
 * fts_ft8006p_wait_state_update - wait fw status update
 */
int fts_ft8006p_wait_state_update(u8 retval)
{
    int ret = 0;
    int times = 0;
    u8 state = 0xFF;
    ...
    while (times++ < FACTORY_TEST_RETRY) {
        ...
        state = 0xFF;
        ret = fts_ft8006p_test_read_reg(FACTORY_REG_PARAM_UPDATE_STATE, &state);
...
/*
 * fts_ft8006p_start_scan - start to scan a frame
 */
int fts_ft8006p_start_scan(void)
{
    int ret = 0;
    u8 addr = 0;
    u8 val = 0;
    ...
    ret = fts_ft8006p_test_write_reg(addr, val);
    if (ret < 0) {
    ...
    while (times++ < FACTORY_TEST_RETRY) {
        ...
        ret = fts_ft8006p_test_read_reg(addr, &val);
    ...
    int *data)
{
    ...
    ret = fts_ft8006p_test_write_reg(off_addr, off_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("wirte line/start addr fail\n");
        return ret;
    }

    ret = fts_ft8006p_read_mass_data(rawdata_addr, byte_num, data);
    if (ret < 0) {
...
int fts_ft8006p_get_rawdata(int *data)
{
    int ret = 0;
    u8 val = 0;
    u8 addr = 0;
    ...
    /* enter factory mode */
    ret = fts_ft8006p_enter_factory_mode();
    if (ret < 0) {
...
/*
 * fts_ft8006p_chip_clb - auto clb
 */
int fts_ft8006p_chip_clb(void)
{
    int ret = 0;
    u8 val = 0;
    int times = 0;
    ...
    /* start clb */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_CLB, 0x04);
    if (ret) {
    ...
    while (times++ < FACTORY_TEST_RETRY) {
        fts_ft8006p_sys_delay(FACTORY_TEST_RETRY_DELAY);
        ret = fts_ft8006p_test_read_reg(FACTORY_REG_CLB, &val);
        if ((0 == ret) && (0x02 == val)) {
            /* clb ok */
            break;
        } else
...
/*
 * fts_ft8006p_get_cb_incell - get cb data for incell IC
 */
int fts_ft8006p_get_cb_incell(u16 saddr, int byte_num, int *cb_buf)
{
    int ret = 0;
    int i = 0;
    u8 cb_addr = 0;
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_ft8006p_test_write_reg(FACTORY_REG_CB_ADDR_H, addr_h);
        if (ret) {
            FTS_TEST_SAVE_ERR("write cb addr high fail\n");
            goto TEST_CB_ERR;
        }
        ret = fts_ft8006p_test_write_reg(FACTORY_REG_CB_ADDR_L, addr_l);
        if (ret) {
            FTS_TEST_SAVE_ERR("write cb addr low fail\n");
            goto TEST_CB_ERR;
        }

        ret = fts_ft8006p_test_read(cb_addr, data + offset, read_num);
        if (ret) {
...
int fts_ft8006p_get_cb_sc(int byte_num, int *cb_buf, enum byte_mode mode)
{
    int ret = 0;
    int i = 0;
    int read_num = 0;
    ...
    cb = (u8 *)fts_ft8006p_malloc(byte_num * sizeof(u8));
    if (NULL == cb) {
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_ft8006p_test_write_reg(off_addr, offset);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("write cb addr offset fail\n");
            goto cb_err;
        }

        ret = fts_ft8006p_test_read(cb_addr, cb + offset, read_num);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("read cb fail\n");
            goto cb_err;
        }
...
bool fts_ft8006p_compare_data(int *data, int min, int max, int min_vk, int max_vk, bool key)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = fts_ft8006p_ftest;
...
bool fts_ft8006p_compare_array(int *data, int *min, int *max, bool key)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = fts_ft8006p_ftest;
...
/*
 * fts_ft8006p_show_data - show and save test data to testresult.txt
 */
void fts_ft8006p_show_data(int *data, bool key)
{
#if TXT_SUPPORT
    int i = 0;
    int j = 0;
...
int fts_ft8006p_mapping_switch(u8 mapping)
{
    int ret = 0;
    u8 val = 0xFF;
    struct fts_test *tdata = fts_ft8006p_ftest;
    ...
    if (tdata->v3_pattern) {
        ret = fts_ft8006p_test_read_reg(FACTORY_REG_NOMAPPING, &val);
        if (ret < 0) {
        ...
        if (val != mapping) {
            ret = fts_ft8006p_test_write_reg(FACTORY_REG_NOMAPPING, mapping);
            if (ret < 0) {
...
bool fts_ft8006p_get_fw_wp(u8 wp_ch_sel, enum wp_type water_proof_type)
{
...
int fts_ft8006p_get_cb_mc_sc(u8 wp, int byte_num, int *cb_buf, enum byte_mode mode)
{
    int ret = 0;

    /* 1:waterproof 0:non-waterproof */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_MC_SC_MODE, wp);
    if (ret < 0) {
    ...
    /* read cb */
    ret = fts_ft8006p_get_cb_sc(byte_num, cb_buf, mode);
    if (ret < 0) {
...
int fts_ft8006p_get_rawdata_mc_sc(enum wp_type wp, int *data)
{
    int ret = 0;
    u8 val = 0;
    u8 addr = 0;
...
int fts_ft8006p_get_rawdata_mc(u8 fre, u8 fir, int *rawdata)
{
    int ret = 0;
    int i = 0;
    ...
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_FRE_LIST, fre);
    if (ret < 0) {
    ...
    /* fir enable/disable */
    ret = fts_ft8006p_test_write_reg(FACTORY_REG_FIR, fir);
    if (ret < 0) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret = fts_ft8006p_get_rawdata(rawdata);
    }
    if (ret < 0) {
...
int fts_ft8006p_short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
    int ret = 0;
    int i = 0;
    u8 short_state = 0;
    ...
    ret = fts_ft8006p_test_write_reg(FACTROY_REG_SHORT_TEST_EN, mode);
    if (ret < 0) {
    ...
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        ...
        ret = fts_ft8006p_test_read_reg(FACTROY_REG_SHORT_TEST_EN, &short_state);
        if ((ret >= 0) && (retval == short_state))
            break;
        else
            FTS_TEST_DBG("reg%x=%x,retry:%d",
    ...
    ret = fts_ft8006p_read_mass_data(FACTORY_REG_SHORT_ADDR_MC, byte_num, adc_buf);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("get short(adc) data fail\n");
    }

    FTS_TEST_DBG("adc data:\n");
    fts_ft8006p_print_buffer(adc_buf, byte_num / 2, 0);
test_err:
    FTS_TEST_FUNC_EXIT();
    return ret;
}

bool fts_ft8006p_compare_mc_sc(bool tx_check, bool rx_check, int *data, int *min, int *max)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = fts_ft8006p_ftest;
...
void fts_ft8006p_show_data_mc_sc(int *data)
{
    int i = 0;
    struct fts_test *tdata = fts_ft8006p_ftest;
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    sprintf(filepath, "%s%s", FTS_SAVE_OPENSHORT_TEST_DATA_FILE_PATH, file_name);
    if (NULL == pfile) {
...
static void fts_ft8006p_test_save_data_csv(struct fts_test *tdata)
{
#if CSV_SUPPORT
    int i = 0;
    int j = 0;
...
/*****************************************************************************
* Name: fts_ft8006p_test_save_data
...
* Return:
*****************************************************************************/
void fts_ft8006p_test_save_data(char *name, int code, int *data, int datacnt,
                        bool mc_sc, bool key, bool result)
{
    ...
    memset(info->name, '\0', TEST_ITEM_NAME_MAX);
    ...
    info->data = fts_ft8006p_malloc(datalen * sizeof(int));
    if (!info->data) {
...
static int get_tx_rx_num(u8 tx_rx_reg, u8 *ch_num, u8 ch_num_max)
{
    ...
    for (i = 0; i < 3; i++) {
        ret = fts_ft8006p_test_read_reg(tx_rx_reg, ch_num);
...
static int get_key_num(int *key_num_en, int max_key_num)
{
    ...
    ret = fts_ft8006p_test_read_reg(FACTORY_REG_LEFT_KEY, &key_en);
    if (ret >= 0) {
        if (key_en & 0x01) {
            (*key_num_en)++;
        }
    ...
    ret = fts_ft8006p_test_read_reg(FACTORY_REG_RIGHT_KEY, &key_en);
    if (ret >= 0) {
        if (key_en & 0x01) {
            (*key_num_en)++;
        }
...
static int fts_test_init_basicinfo(struct fts_test *tdata)
{
    ...
    fts_ft8006p_test_read_reg(REG_FW_VERSION, &val);
    tdata->fw_ver = val;

    if (IC_HW_INCELL == tdata->func->hwtype) {
        fts_ft8006p_test_read_reg(REG_VA_TOUCH_THR, &val);
        tdata->va_touch_thr = val;
        fts_ft8006p_test_read_reg(REG_VKEY_TOUCH_THR, &val);
    ...
    if (IC_HW_MC_SC == tdata->func->hwtype) {
        fts_ft8006p_test_read_reg(FACTORY_REG_PATTERN, &val);
        tdata->v3_pattern = (1 == val) ? true : false;
        fts_ft8006p_test_read_reg(FACTORY_REG_NOMAPPING, &val);
        tdata->mapping = val;
    }
...
static int fts_test_main_init(void)
{
    ...
    tdata->buffer = (int *)fts_ft8006p_malloc(tdata->buffer_length);
...
static int fts_test_main_exit(void)
{
    ...
    FTS_TEST_FUNC_ENTER();
    fts_ft8006p_test_save_data_csv(tdata);
...
static int fts_test_get_testparams(char *config_name)
{
    ...
    ret = fts_ft8006p_test_get_testparam_from_ini(config_name);
...
test_err:
    fts_test_main_exit();
    fts_ft8006p_enter_work_mode();
    return ret;
}

//+ add by songbinbo.wt for tp open short  20190404
int fts_ft8006p_open_short_test(char *ini_file_name)
{
	return fts_test_entry( ini_file_name );
}
EXPORT_SYMBOL(fts_ft8006p_open_short_test);
//- add by songbinbo.wt for tp open short  20190404
...
static int fts_test_func_init(struct fts_ts_data *ts_data)
{
    ...
    struct test_funcs *func = fts_ft8006p_test_func_list[0];
    int func_count = sizeof(fts_ft8006p_test_func_list) / sizeof(fts_ft8006p_test_func_list[0]);
    ...
    fts_ft8006p_ftest = (struct fts_test *)kzalloc(sizeof(*fts_ft8006p_ftest), GFP_KERNEL);
    if (NULL == fts_ft8006p_ftest) {
        FTS_TEST_ERROR("malloc memory for test fail");
        return -ENOMEM;
    }

    for (i = 0; i < func_count; i++) {
        func = fts_ft8006p_test_func_list[i];
        for (j = 0; j < FTX_MAX_COMPATIBLE_TYPE; j++) {
            if (0 == func->ctype[j])
                ...
                fts_ft8006p_ftest->func = func;
            }
        }
    }
    if (NULL == fts_ft8006p_ftest->func) {
        FTS_TEST_ERROR("no test function match, can't test");
        return -ENODATA;
    }

    fts_ft8006p_ftest->ts_data = fts_ft8006p_data;
    return 0;
}

int fts_ft8006p_test_init(struct fts_ts_data *ts_data)
{
    int ret = 0;
...
int fts_ft8006p_test_exit(struct fts_ts_data *ts_data)
{
    FTS_TEST_FUNC_ENTER();

    sysfs_remove_group(&ts_data->dev->kobj, &fts_test_attribute_group);
    fts_free(fts_ft8006p_ftest);
    FTS_TEST_FUNC_EXIT();
    return 0;
}
