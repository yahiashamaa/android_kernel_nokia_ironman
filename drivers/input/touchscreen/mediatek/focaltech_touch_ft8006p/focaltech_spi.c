...
{
    int ret = 0;
    struct spi_device *spi = fts_ft8006p_data->spi;
...
static int fts_wait_idle(void)
{
    ...
    u8 *txbuf = fts_ft8006p_data->bus_tx_buf;
    u8 *rxbuf = fts_ft8006p_data->bus_rx_buf;
    u32 txlen = 0;

    status_mask = 0xB1;
    retry_timeout = 350;
    if (fts_ft8006p_data->fw_is_running)
        idle_status = 0x11;
...
read_data_crc_check_err:

spi_transfer_read_err:
	if (datalen > (SPI_BUF_LENGTH - SPI_HEADER_LENGTH)) {
		if (rxbuf) {
			kfree(rxbuf);
			rxbuf = NULL;
...
{
    return fts_ft8006p_read(&addr, 1, value, 1);
}
