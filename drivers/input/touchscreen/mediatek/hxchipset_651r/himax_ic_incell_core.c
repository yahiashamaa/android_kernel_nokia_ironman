...
static void himax_mcu_read_FW_ver(void)
{
	...
	snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"HX83102-D,VND:LC,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
		...
		int hx_zero_event, int length)
{
	...
	//+bug558651, sheqihao.wt, MODIFY, 20200604, fix tp abnormal after esd test 
			I("[HIMAX TP MSG]: recovery state checked g_is_recovery_done_flag = %d\n", g_is_recovery_done_flag);
			if(g_is_recovery_done_flag) {
				g_is_recovery_done_flag = 0;
