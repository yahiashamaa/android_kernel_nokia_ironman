...
int g_tp_allzero_esdrst_flag;
int g_is_recovery_done_flag; //bug558651, sheqihao.wt, MODIFY, 20200604, fix tp abnormal after esd test 
...
#include <linux/power_supply.h>//+bug591033, sheqihao.wt, ADD, 20201027, Add himax-hx83102d tp charge mode detect
...
//+add by sheqihao.wt for HX83102D Add open short test	20200502
static ssize_t himax_proc_ctp_test(struct seq_file *s, void *v)
{
	int val = 0x00;
	size_t ret = 0;
	...
	if (private_ts->is_openshort_test_going || private_ts->suspended == 1) {
		E("%s: please do self test in normal active mode\n", __func__);
		return HX_INIT_FAIL;
	}
	...
	val = g_core_fp.proc_ctp_chip_test(s, v);
/*
 *#if defined(HX_ESD_RECOVERY)
 *	HX_ESD_RESET_ACTIVATE = 1;
 *#endif
...
static void *himax_proc_ctp_test_seq_start(struct seq_file *s, loff_t *pos)
{
	if (*pos >= 1)
		return NULL;
...
static void *himax_proc_ctp_test_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	return NULL;
}

static void himax_proc_ctp_test_seq_stop(struct seq_file *s, void *v)
{
}


static int himax_proc_ctp_test_seq_read(struct seq_file *s, void *v)
{
	size_t ret = 0;
...
#endif
			seq_puts(s, "No chip test data in proc ctp test.\n");
	} else {
		himax_proc_ctp_test(s, v);
	}

	return ret;
}

static const struct seq_operations himax_proc_ctp_test_seq_ops = {
	.start	= himax_proc_ctp_test_seq_start,
	.next	= himax_proc_ctp_test_seq_next,
	.stop	= himax_proc_ctp_test_seq_stop,
	.show	= himax_proc_ctp_test_seq_read,
};

static int himax_proc_ctp_test_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &himax_proc_ctp_test_seq_ops);
};


static const struct file_operations himax_proc_ctp_test_ops = {
	.owner = THIS_MODULE,
	.open = himax_proc_ctp_test_open,
	.read = seq_read,
	//.write = himax_proc_ctp_test_write,
	.release = seq_release,
};
...
int himax_common_proc_init(void)
{
	...
	himax_proc_self_test_file_wt = proc_create(HIMAX_PROC_SELF_TEST_FILE_WT, 0777, himax_touch_proc_dir_wt, &himax_proc_ctp_test_ops);
...
int himax_input_register(struct himax_ts_data *ts)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 1, 10, 0, 0); //bug558465, sheqihao.wt, MODIFY, 20200601, fix touch jump point
#else
...
/*start ts_work*/
#if defined(HX_USB_DETECT_GLOBAL)
//+bug591033, sheqihao.wt, ADD, 20201027, Add himax-hx83102d tp charge mode detect
/********************************************************************************
*FUNCTION
...
bool charger_detect_status(void){
	...
	if(propval.intval == 1 ) {//POWER_SUPPLY_STATUS_CHARGING
 ...
 */
	...
	if ((hx_esd_event == length || hx_zero_event == length)
		...
		I("[HIMAX TP MSG]ESD: g_tp_allzero_esdrst_flag = %d\n", g_tp_allzero_esdrst_flag);

		if ((shaking_ret == HX_ESD_EVENT)&& (g_tp_allzero_esdrst_flag == 0)){ //bug558651, sheqihao.wt, MODIFY, 20200529, esd test fail
			himax_esd_hw_reset();
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			...
			input_report_abs(ts->input_dev, ABS_MT_PRESSURE,
					g_target_report_data->w[i]);
#else
			input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, i+1);//bug558465, sheqihao.wt, MODIFY, 20200601, fix touch jump point
#endif
...
*  himax_lcm_resume_ts_work_func

*DESCRIPTION

*  lcm resume touch workthread function
...
* NONE
********************************************************************************/
static void himax_lcm_resume_ts_work_func(struct work_struct *work)
{
	himax_chip_common_resume(private_ts);
}
#endif
...
static void himax_suspend_work_func(struct work_struct *work)
{
	I("%s: Enter himax ts recovery work func\n", __func__);

	I("ESD INFO: Start to run tp suspend\n");
...
static void himax_recovery_work_func(struct work_struct *work)
{
	...
	I("ESD INFO: Start to run tp resume\n");
	himax_chip_common_resume(private_ts);
}
...
//+WVR-4220 sheqihao.wt,Modify,20200730,fix phone screen freeze in call scene
#if defined(HX_LCM_RESUME_TS)
	ts->ts_lcm_resume_workqueue =
			create_singlethread_workqueue("himax_ts_lcm_resume_wq");
	if (!ts->ts_lcm_resume_workqueue) {
		E("%s: create ts_lcm_resume workqueue failed\n", __func__);
		goto err_create_ts_lcm_resume_wq_failed;
	}
	INIT_WORK(&ts->ts_lcm_resume_work, himax_lcm_resume_ts_work_func);
#endif
//-WVR-4220 sheqihao.wt,Modify,20200730,fix phone screen freeze in call scene
...
//+bug558651, sheqihao.wt, MODIFY, 20200604, fix tp abnormal after esd test 
	...
	ts->ts_susned_workqueue = create_singlethread_workqueue("himax_ts_suspend_wq");
	if (!ts->ts_susned_workqueue) {
		E("%s: create ts_recovery workqueue failed\n", __func__);
		goto err_create_ts_suspend_wq_failed;
	}
	INIT_WORK(&ts->ts_suspend_work, himax_suspend_work_func);

	ts->ts_recovery_workqueue = create_singlethread_workqueue("himax_ts_recovery_wq");
	if (!ts->ts_recovery_workqueue) {
		E("%s: create ts_recovery workqueue failed\n", __func__);
		goto err_create_ts_recovery_wq_failed;
	}
	INIT_WORK(&ts->ts_recovery_work, himax_recovery_work_func);
...
//+WVR-4220 sheqihao.wt,Modify,20200730,fix phone screen freeze in call scene
#if defined(HX_LCM_RESUME_TS)
err_create_ts_lcm_resume_wq_failed:
	cancel_work_sync(&ts->ts_lcm_resume_work);
	destroy_workqueue(ts->ts_lcm_resume_workqueue);
#endif
//-WVR-4220 sheqihao.wt,Modify,20200730,fix phone screen freeze in call scene
...
err_create_ts_recovery_wq_failed:
	cancel_work_sync(&ts->ts_recovery_work);
	destroy_workqueue(ts->ts_recovery_workqueue);

err_create_ts_suspend_wq_failed:
	cancel_work_sync(&ts->ts_suspend_work);
	destroy_workqueue(ts->ts_susned_workqueue);
