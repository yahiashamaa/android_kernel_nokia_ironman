...
#endif /* PROC_NODE */
    //spi_set_drvdata(spi_fingerprint, fp_dev);
...
err_devt:
    //spi_set_drvdata(spi_fingerprint, NULL);
    fp_dev->spi = NULL;
    kfree(fp_dev);
    fp_dev = NULL;
...
{	
    ...
    if(cdfinger_fp_exist == 1){
 		LOG_MSG_DEBUG(ERR_LOG, "cdfinger sensor has exist cmd = %d\n", cdfinger_fp_exist);
        return -EINVAL;
    } else if(cdfinger_fp_exist == 0){
        LOG_MSG_DEBUG(ERR_LOG, "silead and cdfinger is not cmd = %d\n", cdfinger_fp_exist);
        return -EINVAL;
    }
	...
	LOG_MSG_DEBUG(ERR_LOG, "spi_register_driver enter \n");
#if defined(USE_PLATFORM_BUS)	
	if(spi_fingerprint == NULL){
        class_destroy(silfp_class);
        unregister_chrdev(FP_DEV_MAJOR, silfp_driver.driver.name);
		LOG_MSG_DEBUG(ERR_LOG, "frome cdfinger spi register failed!! \n");
		return -EINVAL;
   } else {
