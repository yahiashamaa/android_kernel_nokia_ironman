...
#endif
{
	...
	mt_spi_enable_master_clk(cdfinger->spi);
#ifdef USE_SPI_BUS
	chipid_tx[0] = 0xFC;    // sileadfp ID
	//chipid_cdfinger_tx[0] = 0x74;  // cdfinger ID 
	for(TestCnt = 0; TestCnt < 5; TestCnt++) {
		cdfinger_fp_exist = 0;
		ret = tee_spi_transfer(chipid_tx, chipid_rx, 4);
		if (ret == 0) {
			CDFINGER_PROBE("get id = (0x%x,0x%x)\n",chipid_rx[2], chipid_rx[3]);
			if ((chipid_rx[2] == 0x18) && (chipid_rx[3] == 0xb0)) {
				CDFINGER_PROBE("get silead id success cnt = (%d) \n", TestCnt);
				cdfinger_fp_exist = -1;
				goto free_cdfinger;
			} else {
				ret = -1;
				...
				if(ret == 0) {
					if(chipid_rx[3] == 0x88 || chipid_rx[3] == 0x98 || chipid_rx[3] == 0x56 || chipid_rx[3] == 0x80) {
						CDFINGER_PROBE("get cdfinger id success = (0x%x) cnt = (%d) \n",chipid_rx[3], TestCnt);
						cdfinger_fp_exist = 1;
						break;
					} else {
						CDFINGER_PROBE("get id fail = (0x%x) cnt = (%d) \n",chipid_rx[3], TestCnt);
						ret = -1;
						cdfinger_reset(10);
						if(TestCnt == 4){
