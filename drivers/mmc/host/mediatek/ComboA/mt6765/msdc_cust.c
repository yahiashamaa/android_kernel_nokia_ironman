...
//bug 596745,liangsuocheng.wt add 2020.11.7 circuit sd power off
struct reg_oc_msdc {
...
void msdc_sd_power(struct msdc_host *host, u32 on)
{
	...
	case 1:
		...
		/* Disable VMCH OC */ //bug 596745,liangsuocheng.wt add 2020.11.7 circuit sd power off
		if (!card_on)
		...
		/* Enable VMCH OC */ //bug 596745,liangsuocheng.wt add 2020.11.7 circuit sd power off
		if (card_on) {
			mdelay(3);
		...
		unsigned long event, void *data)
{
	...
	case REGULATOR_EVENT_FAIL:
		schedule_work(&sd_oc.work);//bug 596745,liangsuocheng.wt add 2020.11.7 circuit sd power off
		break;
	default:
		break;
	};
...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	if (host->id == 1) {
		sd_oc.nb.notifier_call = msdc_sd_event;//bug 596745,liangsuocheng.wt add 2020.11.7 circuit sd power off
