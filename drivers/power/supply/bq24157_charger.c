...
#include <linux/delay.h>    // EXTB IRM-4719,lulongqing.wt,2019.10.28,modify shutdown in reboot test


#include "mtk_charger.h"    //#include "mtk_charger_intf.h" //modify by liujun5
#include "bq24157_reg.h"
//+bug 585485,jinpeng1.wt,2020.9.25,add,charger IC id in factory mode
#include <linux/hardware_info.h>
//-bug 585485,jinpegn1.wt,2020.9.25,add,charger IC id in factory mode
#include <linux/acpi.h>    //#include "../../pmic/include/pmic_api.h" //modify by liujun5
...
//+bug 585485,jinpeng1.wt,2020.10.22,[T99621AA3] charger bring up later problem
#ifdef CONFIG_WT_PROJECT_T99621AA3
static bool is_hl7057_ic = false;
#endif
//-bug 585485,jinpeng1.wt,2020.10.22,[T99621AA3] charger bring up later problem
...
#ifdef CONFIG_WT_PROJECT_T99621AA3
    is_hl7057_ic = true;   //bug 585485,jinpeng1.wt,2020.10.22,modify charging time more than 3h used 2A adapter
#endif
		return KAL_TRUE;
	} else {
		return KAL_FALSE;
...
//+bug 585485,jinpeng1.wt,2020.10.22,[T99621AA3] charger bring up later problem
#ifdef CONFIG_WT_PROJECT_T99621AA3
bool check_hl7057(void)
{
    return is_hl7057_ic;
}
EXPORT_SYMBOL(check_hl7057);
#endif
//-bug 585485,jinpeng1.wt,2020.10.22,[T99621AA3] charger bring up later problem
...
static int bq2415x_detect_device(struct bq2415x *bq)
{
	...
	pr_err("bq2415x:part_no:0x%x revision:0x%x vendor|0x%x \n", 
...
//-BUG IRM-18093,jinpeng1.wt,2020.12.15,modify,improve power_supply node customization to call notify_fg_chg_full
	switch (event) {
	case EVENT_FULL:    //EVENT_EOC: liujun5
...
static int eta6937_dump_register(struct charger_device *chg_dev)
{
	...
	//mt6357_upmu_set_rg_ldo_vcn28_en(1);//delete by liujun5
...
//+bug 585485 ,jinpeng1.wt,2020.10.22,modify charger bring up later problem
#ifdef CONFIG_WT_PROJECT_T99621AA3
    eta6937_disable_charger(bq);
    udelay(500);
    eta6937_enable_charger(bq);
    pr_debug("reset eta6937 charger ic timer\n");
#endif
//-bug 585485,jinpeng1.wt,2020.10.22,modify charger bring up later problem
...
//+BUG IRM-18093,jinpeng1.wt,2020.12.15,modify,improve power_supply node customization to call notify_fg_chg_full
#if defined(CONFIG_WT_PROJECT_T99621AA3) || defined(CONFIG_WT_PROJECT_T99651AA2)
static enum power_supply_property bq2415x_gauge_properties[] = {
...
static int bq2415x_psy_gauge_get_property(struct power_supply *psy,
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	bq = (struct bq2415x *)power_supply_get_drvdata(psy);
	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		ret = bq2415x_read_byte(bq, BQ2415X_REG_00, &status);
		if (!ret) {
			status = status & BQ2415X_STAT_MASK;
			status = status >> BQ2415X_STAT_SHIFT;
			if (status == BQ2415X_STAT_READY) /* Ready */
				val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			else if (status == BQ2415X_STAT_CHARGING) /* Charge in progress */
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
			else if (status == BQ2415X_STAT_CHGDONE) /* Charge done */
				val->intval = POWER_SUPPLY_STATUS_FULL;
			else
				val->intval = POWER_SUPPLY_STATUS_UNKNOWN;

			pr_err("%s switch reg_val:%d\n",__func__, status);
		}else
			return -EINVAL;
		break;
	default:
...
//+bug 585485,jinpeng1.wt,2020.10.22,modify charger bring up later problem
    ...
    {
        bq->cfg.chg_ma = 1750;
        bq->cfg.safety_chg_ma = 1750;
        pr_err("set hl7057 voltage chg_ma=%d,safety_chg_ma=%d\n",bq->cfg.chg_ma,bq->cfg.safety_chg_ma);
    }
#endif
//-bug 585485,jinpeng1.wt,2020.10.22,modify charger bring up later problem
	...
	pr_info("BQ2415x charger driver probe successfully\n");
//+BUG IRM-18093,jinpeng1.wt,2020.12.15,modify,improve power_supply node customization to call notify_fg_chg_full
#if defined(CONFIG_WT_PROJECT_T99621AA3) || defined(CONFIG_WT_PROJECT_T99651AA2)
	bq->psy_desc.name = "bq2415x-gauge";
	bq->psy_desc.type = POWER_SUPPLY_TYPE_USB;
	bq->psy_desc.properties = bq2415x_gauge_properties;
	bq->psy_desc.num_properties = ARRAY_SIZE(bq2415x_gauge_properties);
	bq->psy_desc.get_property = bq2415x_psy_gauge_get_property;
	bq->psy_cfg.drv_data = bq;
	bq->psy_cfg.supplied_to = bq2415x_charger_supplied_to;
	bq->psy_cfg.num_supplicants = ARRAY_SIZE(bq2415x_charger_supplied_to);
