...
int charger_dev_enable_vbus(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->enable_vbus)
		return chg_dev->ops->enable_vbus(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_vbus);
#endif
#endif
//-Bug 597174,lishuwen.wt,2020.11.06,modify,add charger current nod
