...
//	info = (struct mtk_charger_type *)power_supply_get_drvdata(usb_psy);
#endif
//-bug 589756,yaocankun,20201020,add type-c polarity node
...
//+bug 589756,yaocankun,20201020,add type-c polarity node
#if defined(CONFIG_WT_PROJECT_T99653AA1) || defined(CONFIG_WT_PROJECT_T99652AA1)
			propval.intval = noti->typec_state.polarity + 1;
			power_supply_set_property(usb_psy, POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION,
				&propval);
//			info->polarity_state = noti->typec_state.polarity + 1;
//			pr_err("set type-c polarity %d\n", noti->typec_state.polarity);
#endif
