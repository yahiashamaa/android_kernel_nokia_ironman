...
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		...
		do {
			...
			//+Chk37354, sheqihao.wt, MODIFY, 20201127, fix tp abnormal after esd test 
			#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET_651R
				DISPINFO("HIMAX_651R ESD Check\n");
				if ((!ret) && (!g_tp_allzero_esdrst_flag)) /* success */  //bug558651, sheqihao.wt, MODIFY, 20200612, esd test LCM fail
					break;
			#else
				DISPINFO("Normal ESD Check\n");
			...
			#ifdef CONFIG_WT_PROJECT_T99621AA3
				lcd_need_recovery = true;
			#endif

			DISPERR( "[ESD]esd check fail, will do esd recovery. try=%d\n", i);

			#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET_651R
				DISPERR("[ESD]esd check: ret=%d , g_tp_allzero_esdrst_flag=%d\n", ret, g_tp_allzero_esdrst_flag);
			#endif
			...
			#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET_651R
				g_tp_allzero_esdrst_flag = 0;  //bug558651, sheqihao.wt, MODIFY, 20200529, esd test fail
			#endif			
			//-Chk37354, sheqihao.wt, MODIFY, 20201127, fix tp abnormal after esd test 
			recovery_done = 1;
		} while (++i < esd_try_cnt);
...
//+Chk37354, sheqihao.wt, MODIFY, 20201127, fix tp abnormal after esd test 
	#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET_651R
	DISPDBG("[POWER]ts suspend[begin]\n");
	disp_ts_esd_suspend_cmd(primary_get_lcm());
	DISPDBG("[POWER]ts suspend[end]\n");
//-Chk37354, sheqihao.wt, MODIFY, 20201127, fix tp abnormal after esd test 
	#endif
	...
	#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET_651R
	g_is_recovery_done_flag = 1; //Chk37354, sheqihao.wt, MODIFY, 20201127, fix tp abnormal after esd test 
	#endif
