...
#include "rogue_hi1336_rear_txd_mipi_raw.h"
...
static kal_uint16 read_rogue_hi1336_eeprom(kal_uint32 addr)
{
...
static struct imgsensor_info_struct imgsensor_info = { 
	.sensor_id = ROGUE_HI1336_REAR_TXD_SENSOR_ID,
	...
	.cap = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
		//bug 577281 liudijin.wt, ADD, 2020/12/08,  add for 2gong hi1336 dual camera calibration test.
	...
	//+bug596770,liuxiangyin.wt,MODIFY,2020/12/12,update dualcam(hi1336+gc02m1) custom1 setting
...
//+bug596770 zhouyikuan.wt, modify, 202011/11, rogue_hi1336 pdaf porting
...
kal_uint16 addr_data_pair_capture_15fps_hi1336[] = {
	...
	0x0238, 0x1111, //y even/odd inc active
	0x0246, 0x0020, //y read dummy address
	0x020a, 0x1a00, //coarse integ time
	0x021c, 0x0008, //coarse integ time short for iHDR
	0x0206, 0x05dd, //line length pck
	0x020e, 0x1a04, //frame length lines
	0x0b12, 0x1070, //x output size
	0x0b14, 0x0c30, //y output size
	0x0204, 0x0000, //d2a_row_binning_en
	0x041c, 0x0048, //pdaf patch start x-address 
...
//+bug596770,liuxiangyin.wt,MODIFY,2020/12/12,update dualcam(hi1336+gc02m1) custom1 setting
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom1_30fps_hi1336[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
	0x0732, 0xe0b0, //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:000_mipi_div1(1/1) b1-0:00_mipi_div2(1/1)
	0x1118, 0x0006, //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
...
kal_uint16 addr_data_pair_custom1_15fps_hi1336[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:01_isp_div2(1/2)
	0x0732, 0xe0b0, //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:000_mipi_div1(1/1) b1-0:01_mipi_div2(1/2)
	0x1118, 0x0006, //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
...
static void custom1_setting(kal_uint16 currefps){
	...
	if (currefps == 300) {
	hi1336_table_write_cmos_sensor(
		addr_data_pair_custom1_30fps_hi1336,
		sizeof(addr_data_pair_custom1_30fps_hi1336) /
		sizeof(kal_uint16));

	} else {
	hi1336_table_write_cmos_sensor(
		addr_data_pair_custom1_15fps_hi1336,
		sizeof(addr_data_pair_custom1_15fps_hi1336) /
		sizeof(kal_uint16));
	}
#else
  if( currefps == 300) {
...
static struct stCAM_CAL_DATAINFO_STRUCT rogue_hi1336_rear_txd_eeprom_data ={
	.sensorID= ROGUE_HI1336_REAR_TXD_SENSOR_ID,
	.deviceID = 0x01,
	.dataLength = 0xD51,
...
static struct stCAM_CAL_CHECKSUM_STRUCT rogue_hi1336_rear_txd_checksum[8] =
{
	...
	{TOTAL_ITEM,0x0000,0x0000,0x0D4F,0x0D50,0x55},
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//+bug584789,zhouyikuan.wt,ADD,2020/11/03,HI1336 eeprom bring up
				size = imgSensorReadEepromData(&rogue_hi1336_rear_txd_eeprom_data,rogue_hi1336_rear_txd_checksum);
				if(size != rogue_hi1336_rear_txd_eeprom_data.dataLength || (rogue_hi1336_rear_txd_eeprom_data.sensorVendorid >> 24)!= read_rogue_hi1336_eeprom(0x0001)) {
					pr_err("get eeprom data failed\n");
					if(rogue_hi1336_rear_txd_eeprom_data.dataBuffer != NULL) {
						kfree(rogue_hi1336_rear_txd_eeprom_data.dataBuffer);
						rogue_hi1336_rear_txd_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					pr_debug("get eeprom data success\n");
					imgSensorSetEepromData(&rogue_hi1336_rear_txd_eeprom_data);
				}
                                //-bug584789,zhouyikuan.wt,ADD,2020/11/03,HI1336 eeprom bring up
				return ERROR_NONE;
			}
