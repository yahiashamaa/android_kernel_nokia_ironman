...
#include "wr_gc5035_front_qhmipi_Sensor.h"

#undef LOG_INF
#define LIANYI_GC5035_DEBUG 1
#define FLAG_DD_DEBUG 0
#if LIANYI_GC5035_DEBUG
...
#define PFX "lianyi_gc5035_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = WR_GC5035_FRONT_QH_SENSOR_ID,
	...
	//+bug550253 chenbocheng.wt, add, 2020/6/3, add custom1 for front camera portrait mode
	.custom1 = {
		.pclk = 179400000,
		.linelength = 2936,
		.framelength = 2020,
...
static void gc5035_gcore_read_dpc(void)
{
...
#if FLAG_DD_DEBUG
	kal_uint16 k,ddtemp0,ddtemp1,ddtemp2,ddtemp3;
#endif
	...
	case GC5035_OTP_FLAG_VALID: {
		...
		//kal_uint16 k,ddtemp0,ddtemp1,ddtemp2,ddtemp3;
		LOG_ERR("total_num = %d\n", pDPC->total_num);
		for(k=0; k < pDPC->total_num; k++)
		{
		    ddtemp0 = gc5035_otp_read_byte(0x0080+0x0020*k);
			ddtemp1 = gc5035_otp_read_byte(0x0088+0x0020*k);
			ddtemp2 = gc5035_otp_read_byte(0x0090+0x0020*k);
			ddtemp3 = gc5035_otp_read_byte(0x0098+0x0020*k);

			LOG_ERR("GC5035_OTP_DD:index = %d, data[0] = %x , data[1] = %x , data[2] = %x ,data[3] = %x \n",\
				k, ddtemp0, ddtemp1, ddtemp2, ddtemp3);   

            LOG_ERR("GC5035_OTP_DD x = %d , y = %d ,type = %d \n",((ddtemp1&0x0f)<<8) + ddtemp0,
				((ddtemp2&0x7f)<<4) + ((ddtemp1&0xf0)>>4),
				(((ddtemp3&0x03)<<1)+((ddtemp2&0x80)>>7)));
		}
#endif
		break;
	}
...
//+bug550253 chenbocheng.wt, modify, 2020/6/8, the gc5035 otp porting
#define OTP_GROUP_OFFSET 0x40
unsigned char gc5035_data_info[GC5035_OTP_MODULE_DATA_SIZE + 1] = {0};/*Add check sum*/
unsigned char gc5035_data_awb[GC5035_OTP_WB_DATA_SIZE + 1] = {0};/*Add check sum*/
...
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	kal_uint8 otp_grp_flag = 0;
	kal_uint16 check_sum_cal = 0,check_sum = 0;
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		otp_grp_flag = GC5035_OTP_GET_2BIT_FLAG(flag, 2 * (idx));
		if (otp_grp_flag == GC5035_OTP_FLAG_VALID) {
			module_start_offset = (GC5035_OTP_MODULE_DATA_OFFSET) + (OTP_GROUP_OFFSET) * i;
			LOG_INF("the module group:%i is valid,otp_grp_flag = 0x%x,info_start_addr:0x%x\n",(idx+1),otp_grp_flag,module_start_offset);
			break;
		}
	...
	if (otp_grp_flag != GC5035_OTP_FLAG_VALID){
		LOG_INF("the module group:%i is invalid or empty\n",(idx));
		return 0;
		}

	if (module_start_offset != 0) {
		gc5035_otp_read_group(module_start_offset, &gc5035_data_info[0], GC5035_OTP_MODULE_DATA_SIZE + 1);
		for (i = 0; i < GC5035_OTP_MODULE_DATA_SIZE; i++) {
			check_sum_cal += gc5035_data_info[i];
		}
		gc5035_module_id = gc5035_data_info[0];
		position = gc5035_data_info[1];
		lens_id = gc5035_data_info[2];
		vcm_id = gc5035_data_info[3];
		year = gc5035_data_info[4];
		month = gc5035_data_info[5];
		day = gc5035_data_info[6];
		}
	check_sum = gc5035_data_info[GC5035_OTP_MODULE_DATA_SIZE];
	check_sum_cal = (check_sum_cal % 255) + 1;
	if (check_sum_cal == check_sum) {
		LOG_ERR("module info checksum %d pass! check sum = 0x%x, calculate result = 0x%x\n",
			idx + 1, check_sum, check_sum_cal);
	} else {
		LOG_ERR("module info checksum %d error! check sum = 0x%x, calculate result = 0x%x\n",
			idx + 1, check_sum,check_sum_cal);
		return 0;
	}

	LOG_ERR("module_id = 0x%x,position = 0x%x,lens_id = 0x%x,vcm_id = 0x%x\n", gc5035_module_id,position,lens_id,vcm_id);
	LOG_ERR("data = %d-%d-%d\n", year, month, day);

	return 1;
}

static kal_uint8 gc5035_otp_read_wb_info(void)
{
	kal_uint8 otp_grp_flag = 0;
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	...
	kal_uint16 check_sum_awb = 0, check_sum_awb_cal = 0;
	kal_uint16 wb_start_offset = 0;
	kal_uint16 r = 0,b = 0,gr = 0, gb = 0, golden_r = 0, golden_b = 0, golden_gr = 0, golden_gb = 0;
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		otp_grp_flag = GC5035_OTP_GET_2BIT_FLAG(flag, 2 * (idx));
		if (otp_grp_flag == GC5035_OTP_FLAG_VALID) {
			wb_start_offset = (GC5035_OTP_WB_DATA_OFFSET) + (OTP_GROUP_OFFSET) * i;
			LOG_INF("the wb group:%i is valid,otp_grp_flag = 0x%x,wb_start_addr:0x%x\n",(idx+1),otp_grp_flag,wb_start_offset);
			break;
		}
	}

	if (otp_grp_flag != GC5035_OTP_FLAG_VALID){
		LOG_INF("the awb group:%i is invalid or empty\n",idx);
		return 0;
		}

	if (wb_start_offset != 0) {
		gc5035_otp_read_group(wb_start_offset, &gc5035_data_awb[0], GC5035_OTP_WB_DATA_SIZE + 1);
		for (i = 0; i < GC5035_OTP_WB_DATA_SIZE; i++) {
			check_sum_awb_cal += gc5035_data_awb[i];
		}

		r = ((gc5035_data_awb[1]<<8)&0xff00)|(gc5035_data_awb[0]&0xff);
		b = ((gc5035_data_awb[3]<<8)&0xff00)|(gc5035_data_awb[2]&0xff);
		gr = ((gc5035_data_awb[5]<<8)&0xff00)|(gc5035_data_awb[4]&0xff);
		gb = ((gc5035_data_awb[7]<<8)&0xff00)|(gc5035_data_awb[6]&0xff);
		golden_r = ((gc5035_data_awb[9]<<8)&0xff00)|(gc5035_data_awb[8]&0xff);
		golden_b = ((gc5035_data_awb[11]<<8)&0xff00)|(gc5035_data_awb[10]&0xff);
		golden_gr = ((gc5035_data_awb[13]<<8)&0xff00)|(gc5035_data_awb[12]&0xff);
		golden_gb = ((gc5035_data_awb[15]<<8)&0xff00)|(gc5035_data_awb[14]&0xff);
		}

	check_sum_awb = gc5035_data_awb[GC5035_OTP_WB_DATA_SIZE];
	check_sum_awb_cal = (check_sum_awb_cal % 255) + 1;

	if (check_sum_awb_cal == check_sum_awb) {
			LOG_ERR("awb checksum %d pass! check sum = 0x%x, calculate result = 0x%x\n",
				idx + 1, check_sum_awb, check_sum_awb_cal);
			} else {
			LOG_ERR("awb checksum %d error! check sum = 0x%x, calculate result = 0x%x, checksum_addr = 0x%x\n",
				idx + 1, check_sum_awb,check_sum_awb_cal,GC5035_OTP_WB_DATA_SIZE);
			return 0;
		}

	LOG_INF("=== GC5035 AWB r=0x%x, b=0x%x, gr=%x, gb=0x%x ===\n", r, b, gr, gb);
	LOG_INF("=== GC5035 AWB gr=0x%x,gb=0x%x,gGr=%x, gGb=0x%x ===\n", golden_r, golden_b, golden_gr, golden_gb);
...
static kal_uint8 gc5035_otp_read_sensor_info(void)
{
	...
	ret = gc5035_otp_read_module_info();
	if (ret != 1) {
		gc5035_module_id = 0;
		LOG_INF("=== gc5035_otp_read_module_info invalid ===\n");
	} else {
		gc5035_module_id = 1;
	}

	ret = gc5035_otp_read_wb_info();
	if (ret != 1) {
		gc5035_awb_valid = 0;
		LOG_INF("=== gc5035_otp_read_wb_info invalid ===\n");
	} else {
		gc5035_awb_valid = 1;
	}
	...
	if (gc5035_module_id == 0 || gc5035_awb_valid == 0) {
		return -1;
	} else {
		return 0;
	}
...
static void gc5035_otp_update_dd(void)
{
	...
	struct gc5035_dpc_t *pDPC = &gc5035_otp_data.dpc;
#if FLAG_DD_DEBUG
	kal_uint16 i,temp_val0,temp_val1,temp_val2,temp_type,temp;
#endif
	
	LOG_ERR("pDPC->flag=0x%x, pDPC->total_num :%d. \n", pDPC->flag, pDPC->total_num);
	...
	//kal_uint16 i,temp_val0,temp_val1,temp_val2,temp_type,temp;
		...
		write_cmos_sensor(0xaa,0x00);
			...
			for(i=0; i<30; i++)
			{
				//temp = read_cmos_sensor(0xaa);
				temp_val0=read_cmos_sensor(0xac);
				temp_val1=read_cmos_sensor(0xac);
				temp_val2=read_cmos_sensor(0xac);
				temp_type=read_cmos_sensor(0xac);
				temp = read_cmos_sensor(0xaa);
				LOG_ERR("GC5035_OTP_DD temp:%d.\n",temp);
				//CString str;
				LOG_ERR("GC5035_OTP_DD val0 = 0x%x , val1 = 0x%x , val2 = 0x%x,val3 = 0x%x \n",temp_val0,temp_val1,temp_val2,temp_type);
				//fwrite(str,strlen(str),1,fp);
				LOG_ERR("GC5035_OTP_DD x = %d , y = %d ,type = %d \n",((temp_val1&0x0f)<<8) + temp_val0,((temp_val2&0x7f)<<4) + ((temp_val1&0xf0)>>4),(((temp_type&0x01)<<1)+((temp_val2&0x80)>>7)));
				//fwrite(str,strlen(str),1,fp);
			}
...
static void gc5035_otp_update_wb(void)
{
	...
	LOG_ERR("typical rg = 0x%x, bg = 0x%x, pGolden->flag=0x%x.\n", rg_typical, bg_typical, pGolden->flag);
	LOG_ERR("pWB->flag=0x%x.\n", pWB->flag);
...
static void gc5035_otp_update_reg(void)
{
	...
	LOG_INF("reg count = %d, reg flag = 0x%x. \n", gc5035_otp_data.regs.cnt, gc5035_otp_data.regs.flag);
...
static kal_uint8 gc5035_otp_identify(void)
{
	...
	ret = gc5035_otp_read_sensor_info();
...
static void sensor_init(void)
{
	...
	/* SYSTEM */
		...
		write_cmos_sensor(0xf5, 0xe9);
		write_cmos_sensor(0xf6, 0x14);
		write_cmos_sensor(0xf8, 0x45);//49
		write_cmos_sensor(0xf9, 0x82); //key
		write_cmos_sensor(0xfa, 0x00); //key
		write_cmos_sensor(0xfc, 0x81); //key
		write_cmos_sensor(0xfe, 0x00); //key
		write_cmos_sensor(0x36, 0x01); //key
		write_cmos_sensor(0xd3, 0x87); //key
		write_cmos_sensor(0x36, 0x00); //key
		write_cmos_sensor(0x33, 0x00); //key
		write_cmos_sensor(0xfe, 0x03); //key
		write_cmos_sensor(0x01, 0xe7); //key
		write_cmos_sensor(0xf7, 0x01); //key
		write_cmos_sensor(0xfc, 0x8f); //key
		write_cmos_sensor(0xfc, 0x8f); //key
		write_cmos_sensor(0xfc, 0x8e); //key
		write_cmos_sensor(0xfe, 0x00); //key
		write_cmos_sensor(0xee, 0x30); //key
		write_cmos_sensor(0x87, 0x18); //key
		write_cmos_sensor(0xfe, 0x01); //key
		write_cmos_sensor(0x8c, 0x90); //key
		write_cmos_sensor(0xfe, 0x00); //key
		...
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0xfc, 0x88); //key
		write_cmos_sensor(0xfe, 0x10); //key
		write_cmos_sensor(0xfe, 0x00); //key
		write_cmos_sensor(0xfc, 0x8e); //key
		write_cmos_sensor(0xfe, 0x00); //key
		write_cmos_sensor(0xfe, 0x00); //key
		...
		write_cmos_sensor(0xfe, 0x01); //key
		write_cmos_sensor(0x8c, 0x10); //key
		write_cmos_sensor(0xfe, 0x00); //key
		write_cmos_sensor(0x3e, 0x91); //key
}
...
static void preview_setting(void)
{
		LOG_INF("E\n");
		/* System */
		write_cmos_sensor(0xfc, 0x01); //key
		write_cmos_sensor(0xf4, 0x40); //key
		write_cmos_sensor(0xf5, 0xe4); //key
		write_cmos_sensor(0xf6, 0x14); //key
		write_cmos_sensor(0xf8, 0x45); //key
		write_cmos_sensor(0xf9, 0x12); //key
		write_cmos_sensor(0xfa, 0x01); //key
		write_cmos_sensor(0xfc, 0x81); //key
		...
		write_cmos_sensor(0xd3, 0x87); //key
		write_cmos_sensor(0x36, 0x00); //key
		write_cmos_sensor(0x33, 0x20); //key
		write_cmos_sensor(0xfe, 0x03); //key
		write_cmos_sensor(0x01, 0x87); //key
								  
		write_cmos_sensor(0xf7, 0x11); //key
		write_cmos_sensor(0xfc, 0x8f); //key
		write_cmos_sensor(0xfc, 0x8f); //key
		write_cmos_sensor(0xfc, 0x8e); //key
...
static void hs_video_setting(void)
{
	...
	/* System */
		write_cmos_sensor(0xfc, 0x01); //key
		write_cmos_sensor(0xf4, 0x40); //key
		write_cmos_sensor(0xf5, 0xe9); //key
		write_cmos_sensor(0xf6, 0x14); //key
		write_cmos_sensor(0xf8, 0x45); //key
		write_cmos_sensor(0xf9, 0x82); //key
		write_cmos_sensor(0xfa, 0x00); //key
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				ret = gc5035_otp_identify();
				if ( ret == 0) {
					LOG_INF("get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					return ERROR_NONE;
				}else {
					*sensor_id = 0xFFFFFFFF;
					LOG_INF("GC5035 read OTP:NOK!");
					return ERROR_SENSOR_CONNECT_FAIL;
				}
			}
			LOG_ERR("get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_ERR("open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 get_resolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *sensor_resolution)
{
	...
	//+bug550253 chenbocheng.wt, add, 2020/6/3, add custom1 for front camera portrait mode
	sensor_resolution->SensorFullWidth = imgsensor_info.custom1.grabwindow_width;
	sensor_resolution->SensorFullHeight = imgsensor_info.custom1.grabwindow_height;
	//-bug550253 chenbocheng.wt, add, 2020/6/3, add custom1 for front camera portrait mode
