 ...
 * Filename:
 * ---------
 *	 rogue_s5k3l6_rear_dcmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "rogue_s5k3l6_rear_dcmipiraw_Sensor.h"

/*===FEATURE SWITH===*/
#define FPT_PDAF_SUPPORT   //for pdaf switch
#define MT6765

//#define DUALCAM_PDAF_SUPPORT

#define MULTI_WRITE 1


//+bug584789,zhouyikuan.wt,ADD,2020/11/02,S5K3L6 eeprom bring up with checksum
#define EEPROM_P24C64EC4_ID 0xA0

static kal_uint16 s5k3l6_read_eeprom(kal_uint32 addr)
{
	kal_uint16 get_byte=0;

	char pu_send_cmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
	iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 1, EEPROM_P24C64EC4_ID);
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = ROGUE_S5K3L6_REAR_DC_SENSOR_ID,
	...
	.cap = {
		...
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,
		//bug 577281 liudijin.wt, ADD, 2020/11/06,  add for T99652AA1 dual camera calibration test.
	...
	.custom1 = {
		...
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,
         //bug596770 zhouyikuan.wt, modify, 2020/11/23/,fix camera freeze when dump process raw
         //.mipi_pixel_rate = 480000000,
	},
#else
	//8 M * 24 fps
	.custom1 = {
		.pclk = 480000000,				//record different mode's pclk
		.linelength  = 5280,//5808,				//record different mode's linelength
...
//-bug550253 chenbocheng.wt, add, 2020/6/3, add custom2 for rear camera nightscape mode
	...
	.custom2_delay_frame = 2,
        ...
        //bug596770 zhouyikuan.wt, modify, 2020/11/11,fix s53l6 camera RF interference
	...
	.i2c_speed = 400, /*support Max speed 400kHz write*/
...
#endif
//+bug550253 chenbocheng.wt, add, 2020/6/28, add 8m pdaf feature for dualcam
#ifdef DUALCAM_PDAF_SUPPORT
static struct SET_PD_BLOCK_INFO_T imgsensor_dualcam_pd_info =
//for 3L6 8M
{
	.i4OffsetX = 0,
	...
	.i4PosL = { {4, 15}, {56, 15}, {20, 19}, {40, 19}, {8, 35}, {52, 35}, {24, 39}, {36, 39}, {24, 47}, {36, 47}, {8, 51}, {52, 51}, {20, 67}, {40, 67}, {4, 71}, {56, 71} },

	.i4PosR = { {4, 19}, {56, 19}, {20, 23}, {40, 23}, {8, 31}, {52, 31}, {24, 35}, {36, 35}, {24, 51}, {36, 51}, {8, 55}, {52, 55}, {20, 63}, {40, 63}, {4, 67}, {56, 67} },
...
{
	pr_debug("custom2_setting() use custom1\n");
...
static struct stCAM_CAL_DATAINFO_STRUCT dongci_rear_s5k3l6_eeprom_data ={
	.sensorID= ROGUE_S5K3L6_REAR_DC_SENSOR_ID,
	.deviceID = 0x01,
	.dataLength = 0x0CFB,
...
static struct stCAM_CAL_CHECKSUM_STRUCT dongci_rear_s5k3l6_Checksum[8] =
{
	...
	{PDAF_ITEM,0x0770,0x0770,0x0960,0x0961,0x55},
        {PDAF_PROC2_ITEM,0x0962,0x0962,0x0CF8,0x0CF9,0x55},
	{TOTAL_ITEM,0x0000,0x0000,0x0CF9,0x0CFA,0x55},
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) |
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
                                ...
                                //+bug584789,zhouyikuan.wt,ADD,2020/11/02,S5K3L6 eeprom bring up with checksum
				size = imgSensorReadEepromData(&dongci_rear_s5k3l6_eeprom_data,dongci_rear_s5k3l6_Checksum);
				if(size != dongci_rear_s5k3l6_eeprom_data.dataLength || (dongci_rear_s5k3l6_eeprom_data.sensorVendorid >> 24)!= s5k3l6_read_eeprom(0x0001)) {
					pr_err("get eeprom data failed\n");
					if(dongci_rear_s5k3l6_eeprom_data.dataBuffer != NULL) {
						kfree(dongci_rear_s5k3l6_eeprom_data.dataBuffer);
						dongci_rear_s5k3l6_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					pr_debug("get eeprom data success\n");
					imgSensorSetEepromData(&dongci_rear_s5k3l6_eeprom_data);
				}
                                //-bug584789,zhouyikuan.wt,ADD,2020/11/02,S5K3L6 eeprom bring up with checksum
				return ERROR_NONE;
			}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW: /*2x2 binning*/
                //+bug584789 zhouyikuan.wt, MODIFY, 2020/11/03,add pdaf feature for dualcam
                case MSDK_SCENARIO_ID_CUSTOM1:
			memcpy(
				(void *)PDAFinfo,
...
//+bug550253 chenbocheng.wt, add, 2020/6/28, add 8m pdaf feature for dualcam
		...
		case MSDK_SCENARIO_ID_CUSTOM1:
			memcpy(
				(void *)PDAFinfo,
				(void *)&imgsensor_dualcam_pd_info,
				sizeof(struct SET_PD_BLOCK_INFO_T));
			break;
#endif
//-bug550253 chenbocheng.wt, add, 2020/6/28, add 8m pdaf feature for dualcam
...
UINT32 ROGUE_S5K3L6_REAR_DC_MIPI_RAW_SensorInit(
