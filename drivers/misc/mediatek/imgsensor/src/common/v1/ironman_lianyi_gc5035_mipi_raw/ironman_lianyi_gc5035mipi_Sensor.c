...
#include "ironman_lianyi_gc5035mipi_Sensor.h"

#undef LOG_INF
#define LIANYI_GC5035_DEBUG 0
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IRONMAN_LIANYI_GC5035_SENSOR_ID,
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((check % 255 + 1) == info[GC5035_OTP_MODULE_DATA_SIZE - 1]) {
				...
				LOG_ERR("module_id = 0x%x\n", module_info.module_id);
				LOG_ERR("lens_id = 0x%x\n", module_info.lens_id);
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((golden_check % 255 + 1) == golden[GC5035_OTP_GOLDEN_DATA_SIZE - 1]) {
				...
				LOG_ERR("golden flag= 0x%x\n", pGolden->flag);
				LOG_ERR("golden r/g = 0x%x\n", pGolden->rg);
				LOG_ERR("golden b/g = 0x%x\n", pGolden->bg);
			} else {
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
                                    moduleId = gc5035_otp_identify();
                                        if ( moduleId == LIANYI_GC5035_OTP_MODULE_ID) {
                                                        imgsensor_info.module_id = LIANYI_GC5035_OTP_MODULE_ID;
                                                        LOG_INF("get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x, module id:0x%x. \n", imgsensor.i2c_write_id, *sensor_id, moduleId);
                                                        return ERROR_NONE;
					}
			}
			LOG_ERR("get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x, module id:0x%x.\n", imgsensor.i2c_write_id, *sensor_id, moduleId);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id || moduleId != LIANYI_GC5035_OTP_MODULE_ID) {
