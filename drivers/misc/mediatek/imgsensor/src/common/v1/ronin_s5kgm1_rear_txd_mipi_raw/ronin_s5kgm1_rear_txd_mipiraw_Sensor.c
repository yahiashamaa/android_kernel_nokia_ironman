 ...
 * Filename:
 * ---------
 *	 ronin_s5kgm1_rear_txdmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "RONIN_S5KGM1_REAR_TXD_camera_sensor"
...
#include "ronin_s5kgm1_rear_txd_mipiraw_Sensor.h"
...
static kal_uint16 gm1_read_eeprom(kal_uint32 addr)
{
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = RONIN_S5KGM1_REAR_TXD_SENSOR_ID,
	...
	.pre = {
		...
		//bug 577281 liudijin.wt, ADD, 2020/11/05,  add for dual camera calibration test.
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if(shutter > 65530) {  //linetime=10160/960000000<< maxshutter=3023622-line=32s
		...
		printk(KERN_ERR "long_shutter = %d\n", long_shutter);
		pr_debug("Calc long exposure  -\n");
...
static struct stCAM_CAL_DATAINFO_STRUCT txd_rear_s5kgm1_eeprom_data ={
	.sensorID= RONIN_S5KGM1_REAR_TXD_SENSOR_ID,
	.deviceID = 0x01,
	.dataLength = 0x0D51,
...
static struct stCAM_CAL_CHECKSUM_STRUCT txd_rear_s5kgm1_Checksum[8] =
{
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) |
			...
			imgsensor.i2c_write_id, *sensor_id);
				...
				//+bug584789 chenbocheng.wt, add, 2020/10/30, add main camera s5kgm1 and wide camera hi556 otp code
				size = imgSensorReadEepromData(&txd_rear_s5kgm1_eeprom_data,txd_rear_s5kgm1_Checksum);
				if(size != txd_rear_s5kgm1_eeprom_data.dataLength || (txd_rear_s5kgm1_eeprom_data.sensorVendorid >> 24)!= gm1_read_eeprom(0x0001)) {
					pr_err("get eeprom data failed\n");
					if(txd_rear_s5kgm1_eeprom_data.dataBuffer != NULL) {
						kfree(txd_rear_s5kgm1_eeprom_data.dataBuffer);
						txd_rear_s5kgm1_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					pr_info("get eeprom data success\n");
					imgSensorSetEepromData(&txd_rear_s5kgm1_eeprom_data);
				}
				//-bug584789 chenbocheng.wt, add, 2020/10/30, add main camera s5kgm1 and wide camera hi556 otp code
...
static kal_uint32 open(void)
{
	...
	PK_DBG("%s", __func__);
	PK_DBG("%s,s5kgm1_mipi_test:mclk current 4mA\n", __func__);
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = RONIN_S5KGM1_REAR_TXD_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_ronin_s5kgm1_rear_txd, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_ronin_s5kgm1_rear_txd, sizeof(MUINT32)*4);
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_ronin_s5kgm1_rear_txd[%d]= 0x%x\n", i, sn_inf_main_ronin_s5kgm1_rear_txd[i]);
						}
				#endif
			}
				break;
...
UINT32 RONIN_S5KGM1_REAR_TXD_MIPI_RAW_SensorInit(
